...
var _ datasource.DataSource = &RecordRpzSrvDataSource{}

func NewRecordRpzSrvDataSource() datasource.DataSource {
	return &RecordRpzSrvDataSource{}
}

// RecordRpzSrvDataSource defines the data source implementation.
type RecordRpzSrvDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzSrvDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_srv"
}

type RecordRpzSrvModelWithFilter struct {
...
func (m *RecordRpzSrvModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzSrv, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzSrvAttrTypes, diags, FlattenRecordRpzSrv)
}

func (d *RecordRpzSrvDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RPZ SRV Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzSrvResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzSrvDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzSrvDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzSrvModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzSrv, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read RecordRpzSrv by extattrs, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListRecordRpzSrvResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzSrvResponseObject.AdditionalProperties

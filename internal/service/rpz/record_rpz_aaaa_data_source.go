...
var _ datasource.DataSource = &RecordRpzAaaaDataSource{}

func NewRecordRpzAaaaDataSource() datasource.DataSource {
	return &RecordRpzAaaaDataSource{}
}

// RecordRpzAaaaDataSource defines the data source implementation.
type RecordRpzAaaaDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzAaaaDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_aaaa"
}

type RecordRpzAaaaModelWithFilter struct {
...
func (m *RecordRpzAaaaModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzAaaa, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzAaaaAttrTypes, diags, FlattenRecordRpzAaaa)
}

func (d *RecordRpzAaaaDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RPZ AAAA Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzAaaaResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzAaaaDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzAaaaDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzAaaaModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzAaaa, string, error) {
			...
			res := apiRes.ListRecordRpzAaaaResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzAaaaResponseObject.AdditionalProperties

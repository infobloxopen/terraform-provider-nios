...
func ExpandExtAttrs(ctx context.Context, extattrs types.Map, diags *diag.Diagnostics) *map[string]rpz.ExtAttrs {
	...
	result := make(map[string]rpz.ExtAttrs)

	for key, valStr := range extAttrsMap {
		parsedValue := utils.ParseInterfaceValue(valStr)
		result[key] = rpz.ExtAttrs{Value: parsedValue}
	}
	return &result
}

func FlattenExtAttrs(ctx context.Context, planExtAttrs types.Map, extattrs *map[string]rpz.ExtAttrs, diags *diag.Diagnostics) types.Map {
...
func RemoveInheritedExtAttrs(ctx context.Context, planExtAttrs types.Map, respExtAttrs map[string]rpz.ExtAttrs) (*map[string]rpz.ExtAttrs, types.Map, diag.Diagnostics) {
	var diags diag.Diagnostics
	var planMap map[string]rpz.ExtAttrs
	extAttrsRespMap := make(map[string]rpz.ExtAttrs, len(planExtAttrs.Elements()))
	extAttrsAllRespMap := make(map[string]rpz.ExtAttrs)
	var extAttrAll types.Map

	if planExtAttrs.IsNull() || planExtAttrs.IsUnknown() {
		planMap = make(map[string]rpz.ExtAttrs)
	} else {

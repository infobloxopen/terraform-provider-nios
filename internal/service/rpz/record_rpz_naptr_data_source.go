...
var _ datasource.DataSource = &RecordRpzNaptrDataSource{}

func NewRecordRpzNaptrDataSource() datasource.DataSource {
	return &RecordRpzNaptrDataSource{}
}

// RecordRpzNaptrDataSource defines the data source implementation.
type RecordRpzNaptrDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzNaptrDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_naptr"
}

type RecordRpzNaptrModelWithFilter struct {
...
func (m *RecordRpzNaptrModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzNaptr, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzNaptrAttrTypes, diags, FlattenRecordRpzNaptr)
}

func (d *RecordRpzNaptrDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RPZ NAPTR Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzNaptrResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzNaptrDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzNaptrDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzNaptrModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzNaptr, string, error) {
			...
			res := apiRes.ListRecordRpzNaptrResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzNaptrResponseObject.AdditionalProperties

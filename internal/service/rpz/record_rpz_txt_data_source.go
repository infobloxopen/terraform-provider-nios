...
var _ datasource.DataSource = &RecordRpzTxtDataSource{}

func NewRecordRpzTxtDataSource() datasource.DataSource {
	return &RecordRpzTxtDataSource{}
}

// RecordRpzTxtDataSource defines the data source implementation.
type RecordRpzTxtDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzTxtDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_txt"
}

type RecordRpzTxtModelWithFilter struct {
...
func (m *RecordRpzTxtModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzTxt, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzTxtAttrTypes, diags, FlattenRecordRpzTxt)
}

func (d *RecordRpzTxtDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RPZ TXT Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzTxtResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzTxtDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzTxtDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzTxtModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzTxt, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read RecordRpzTxt by extattrs, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListRecordRpzTxtResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzTxtResponseObject.AdditionalProperties

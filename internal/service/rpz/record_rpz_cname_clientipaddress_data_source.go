...
var _ datasource.DataSource = &RecordRpzCnameClientipaddressDataSource{}

func NewRecordRpzCnameClientipaddressDataSource() datasource.DataSource {
	return &RecordRpzCnameClientipaddressDataSource{}
}

// RecordRpzCnameClientipaddressDataSource defines the data source implementation.
type RecordRpzCnameClientipaddressDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzCnameClientipaddressDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_cname_clientipaddress"
}

type RecordRpzCnameClientipaddressModelWithFilter struct {
...
func (m *RecordRpzCnameClientipaddressModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzCnameClientipaddress, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzCnameClientipaddressAttrTypes, diags, FlattenRecordRpzCnameClientipaddress)
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzCnameClientipaddressResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzCnameClientipaddressDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzCnameClientipaddressDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzCnameClientipaddressModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzCnameClientipaddress, string, error) {
			...
			res := apiRes.ListRecordRpzCnameClientipaddressResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzCnameClientipaddressResponseObject.AdditionalProperties

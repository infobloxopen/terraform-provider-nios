...
var _ datasource.DataSource = &RecordRpzCnameIpaddressDataSource{}

func NewRecordRpzCnameIpaddressDataSource() datasource.DataSource {
	return &RecordRpzCnameIpaddressDataSource{}
}

// RecordRpzCnameIpaddressDataSource defines the data source implementation.
type RecordRpzCnameIpaddressDataSource struct {
...
func (m *RecordRpzCnameIpaddressModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzCnameIpaddress, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzCnameIpaddressAttrTypes, diags, FlattenRecordRpzCnameIpaddress)
}

func (d *RecordRpzCnameIpaddressDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieve RPZ CNAME IP address record.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzCnameIpaddressResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzCnameIpaddressDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzCnameIpaddressDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzCnameIpaddressModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzCnameIpaddress, string, error) {
			...
			res := apiRes.ListRecordRpzCnameIpaddressResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzCnameIpaddressResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &RecordRpzCnameClientipaddressdnDataSource{}

func NewRecordRpzCnameClientipaddressdnDataSource() datasource.DataSource {
	return &RecordRpzCnameClientipaddressdnDataSource{}
}

// RecordRpzCnameClientipaddressdnDataSource defines the data source implementation.
type RecordRpzCnameClientipaddressdnDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzCnameClientipaddressdnDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_cname_clientipaddressdn"
}

type RecordRpzCnameClientipaddressdnModelWithFilter struct {
...
func (m *RecordRpzCnameClientipaddressdnModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzCnameClientipaddressdn, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzCnameClientipaddressdnAttrTypes, diags, FlattenRecordRpzCnameClientipaddressdn)
}

func (d *RecordRpzCnameClientipaddressdnDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RPZ CNAME Client IP Address DN records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzCnameClientipaddressdnResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzCnameClientipaddressdnDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzCnameClientipaddressdnDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzCnameClientipaddressdnModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzCnameClientipaddressdn, string, error) {
			...
			res := apiRes.ListRecordRpzCnameClientipaddressdnResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzCnameClientipaddressdnResponseObject.AdditionalProperties

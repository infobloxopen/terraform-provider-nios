...
var _ datasource.DataSource = &RecordRpzPtrDataSource{}

func NewRecordRpzPtrDataSource() datasource.DataSource {
	return &RecordRpzPtrDataSource{}
}

// RecordRpzPtrDataSource defines the data source implementation.
type RecordRpzPtrDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzPtrDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_ptr"
}

type RecordRpzPtrModelWithFilter struct {
...
func (m *RecordRpzPtrModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzPtr, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzPtrAttrTypes, diags, FlattenRecordRpzPtr)
}

func (d *RecordRpzPtrDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RPZ PTR Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzPtrResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzPtrDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzPtrDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzPtrModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzPtr, string, error) {
			...
			res := apiRes.ListRecordRpzPtrResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzPtrResponseObject.AdditionalProperties

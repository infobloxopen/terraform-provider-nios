...
// RecordRpzAaaaIpaddressDataSource defines the data source implementation.
type RecordRpzAaaaIpaddressDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzAaaaIpaddressDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_aaaa_ipaddress"
}

type RecordRpzAaaaIpaddressModelWithFilter struct {
...
func (m *RecordRpzAaaaIpaddressModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzAaaaIpaddress, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzAaaaIpaddressAttrTypes, diags, FlattenRecordRpzAaaaIpaddress)
}

func (d *RecordRpzAaaaIpaddressDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RPZ AAAA IPADDRESS Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzAaaaIpaddressResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzAaaaIpaddressDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzAaaaIpaddressDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzAaaaIpaddressModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzAaaaIpaddress, string, error) {
			...
			res := apiRes.ListRecordRpzAaaaIpaddressResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzAaaaIpaddressResponseObject.AdditionalProperties

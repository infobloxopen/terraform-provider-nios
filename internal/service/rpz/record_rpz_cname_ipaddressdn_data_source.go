...
var _ datasource.DataSource = &RecordRpzCnameIpaddressdnDataSource{}

func NewRecordRpzCnameIpaddressdnDataSource() datasource.DataSource {
	return &RecordRpzCnameIpaddressdnDataSource{}
}

// RecordRpzCnameIpaddressdnDataSource defines the data source implementation.
type RecordRpzCnameIpaddressdnDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzCnameIpaddressdnDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_cname_ipaddressdn"
}

type RecordRpzCnameIpaddressdnModelWithFilter struct {
...
func (m *RecordRpzCnameIpaddressdnModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzCnameIpaddressdn, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzCnameIpaddressdnAttrTypes, diags, FlattenRecordRpzCnameIpaddressdn)
}

func (d *RecordRpzCnameIpaddressdnDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RPZ CNAME IP Address DN records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzCnameIpaddressdnResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzCnameIpaddressdnDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzCnameIpaddressdnDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzCnameIpaddressdnModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzCnameIpaddressdn, string, error) {
			...
			res := apiRes.ListRecordRpzCnameIpaddressdnResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzCnameIpaddressdnResponseObject.AdditionalProperties

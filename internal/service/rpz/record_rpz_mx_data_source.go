...
var _ datasource.DataSource = &RecordRpzMxDataSource{}

func NewRecordRpzMxDataSource() datasource.DataSource {
	return &RecordRpzMxDataSource{}
}

// RecordRpzMxDataSource defines the data source implementation.
type RecordRpzMxDataSource struct {
	client *niosclient.APIClient
}

func (d *RecordRpzMxDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "rpz_record_mx"
}

type RecordRpzMxModelWithFilter struct {
...
func (m *RecordRpzMxModelWithFilter) FlattenResults(ctx context.Context, from []rpz.RecordRpzMx, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RecordRpzMxAttrTypes, diags, FlattenRecordRpzMx)
}

func (d *RecordRpzMxDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RPZ MX Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RecordRpzMxResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RecordRpzMxDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RecordRpzMxDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RecordRpzMxModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rpz.RecordRpzMx, string, error) {
			...
			res := apiRes.ListRecordRpzMxResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRecordRpzMxResponseObject.AdditionalProperties

...
func ExpandExtAttr(ctx context.Context, tfMap types.Map, diags *diag.Diagnostics) *map[string]discovery.ExtAttrs {
	if tfMap.IsNull() || tfMap.IsUnknown() {
		return nil
	}

	result := make(map[string]discovery.ExtAttrs)
...
func (m *ExtAttrModel) Expand(ctx context.Context, diags *diag.Diagnostics) *discovery.ExtAttrs {
	if m == nil {
		return nil
	}
	to := &discovery.ExtAttrs{
		Value: flex.ExpandString(m.Value),
	}
	return to
}

func FlattenExtAttr(ctx context.Context, input map[string]discovery.ExtAttrs, diags *diag.Diagnostics) types.Map {
...
func (m *ExtAttrModel) Flatten(ctx context.Context, from *discovery.ExtAttrs, diags *diag.Diagnostics) {
	if from == nil {
		return
	}
	if m == nil {
...
func RemoveInheritedExtAttrs(ctx context.Context, planExtAttrs types.Map, respExtAttrs map[string]discovery.ExtAttrs) (*map[string]discovery.ExtAttrs, diag.Diagnostics) {
	var diags diag.Diagnostics
	newRespMap := make(map[string]discovery.ExtAttrs, len(respExtAttrs))

...
// DiscoveryCredentialgroupDataSource defines the data source implementation.
type DiscoveryCredentialgroupDataSource struct {
	client *niosclient.APIClient
}

func (d *DiscoveryCredentialgroupDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "discovery_credentialgroup"
}

type DiscoveryCredentialgroupModelWithFilter struct {
...
func (m *DiscoveryCredentialgroupModelWithFilter) FlattenResults(ctx context.Context, from []discovery.DiscoveryCredentialgroup, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, DiscoveryCredentialgroupAttrTypes, diags, FlattenDiscoveryCredentialgroup)
}

func (d *DiscoveryCredentialgroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Discovery Credential Groups.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(DiscoveryCredentialgroupResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *DiscoveryCredentialgroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DiscoveryCredentialgroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data DiscoveryCredentialgroupModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]discovery.DiscoveryCredentialgroup, string, error) {
			...
			res := apiRes.ListDiscoveryCredentialgroupResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListDiscoveryCredentialgroupResponseObject.AdditionalProperties

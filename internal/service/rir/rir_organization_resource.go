...
func (r *RirOrganizationResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	var diags diag.Diagnostics
	var data RirOrganizationModel
	var goClientData rir.RirOrganization
	...
	updateRes, _, err := r.client.RIRAPI.
		RirOrganizationAPI.
		Update(ctx, resourceRef).
		RirOrganization(goClientData).
		ReturnFieldsPlus(readableAttributesForRirOrganization).
		ReturnAsObject(1).
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Import Failed", fmt.Sprintf("Unable to update RirOrganization for import, got error: %s", err))
		return
	}

	res := updateRes.UpdateRirOrganizationResponseAsObject.GetResult()

	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrsAll, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update RirOrganization due inherited Extensible attributes for import, got error: %s", diags))
		return
	}

...
var _ datasource.DataSource = &RirOrganizationDataSource{}

func NewRirOrganizationDataSource() datasource.DataSource {
	return &RirOrganizationDataSource{}
}

// RirOrganizationDataSource defines the data source implementation.
type RirOrganizationDataSource struct {
	client *niosclient.APIClient
}

func (d *RirOrganizationDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (m *RirOrganizationModelWithFilter) FlattenResults(ctx context.Context, from []rir.RirOrganization, diags *diag.Diagnostics) {
	...
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RirOrganizationAttrTypes, diags, FlattenRirOrganization)
}

func (d *RirOrganizationDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing RIR Organizations.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RirOrganizationResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RirOrganizationDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RirOrganizationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RirOrganizationModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]rir.RirOrganization, string, error) {
			...
			res := apiRes.ListRirOrganizationResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRirOrganizationResponseObject.AdditionalProperties

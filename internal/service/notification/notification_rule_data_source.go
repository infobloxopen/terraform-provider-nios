...
// NotificationRuleDataSource defines the data source implementation.
...
func (m *NotificationRuleModelWithFilter) FlattenResults(ctx context.Context, from []notification.NotificationRule, diags *diag.Diagnostics) {
	...
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, NotificationRuleAttrTypes, diags, FlattenNotificationRule)
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(NotificationRuleResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *NotificationRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data NotificationRuleModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]notification.NotificationRule, string, error) {
			...
			res := apiRes.ListNotificationRuleResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListNotificationRuleResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &NotificationRestEndpointDataSource{}

func NewNotificationRestEndpointDataSource() datasource.DataSource {
	return &NotificationRestEndpointDataSource{}
}

// NotificationRestEndpointDataSource defines the data source implementation.
type NotificationRestEndpointDataSource struct {
	client *niosclient.APIClient
}

func (d *NotificationRestEndpointDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "notification_rest_endpoint"
}

type NotificationRestEndpointModelWithFilter struct {
...
func (m *NotificationRestEndpointModelWithFilter) FlattenResults(ctx context.Context, from []notification.NotificationRestEndpoint, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, NotificationRestEndpointAttrTypes, diags, FlattenNotificationRestEndpoint)
}

func (d *NotificationRestEndpointDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Notification REST Endpoints.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(NotificationRestEndpointResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *NotificationRestEndpointDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NotificationRestEndpointDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data NotificationRestEndpointModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]notification.NotificationRestEndpoint, string, error) {
			...
			request := d.client.NotificationAPI.
			...
			res := apiRes.ListNotificationRestEndpointResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListNotificationRestEndpointResponseObject.AdditionalProperties

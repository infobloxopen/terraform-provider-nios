...
func ExpandExtAttrs(ctx context.Context, extattrs types.Map, diags *diag.Diagnostics) *map[string]notification.ExtAttrs {
	...
	result := make(map[string]notification.ExtAttrs)

	for key, valStr := range extAttrsMap {
		parsedValue := utils.ParseInterfaceValue(valStr)
		result[key] = notification.ExtAttrs{Value: parsedValue}
	}
	return &result
}

func FlattenExtAttrs(ctx context.Context, planExtAttrs types.Map, extattrs *map[string]notification.ExtAttrs, diags *diag.Diagnostics) types.Map {

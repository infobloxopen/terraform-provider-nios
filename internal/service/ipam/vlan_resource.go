...
var _ resource.ResourceWithIdentity = &VlanResource{}
...
func (r *VlanResource) IdentitySchema(ctx context.Context, req resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *VlanResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	var apiRes *ipam.CreateVlanResponse
	...
	if diags.HasError() {
		resp.Diagnostics.Append(diags...)
		resp.Diagnostics.AddError("Client Error", "Error while creating Vlan due to inherited Extensible attributes")
		return
	}
...
func (r *VlanResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if associateInternalId == nil {
		...
		if stateExtAttrs == nil {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read Vlan because the internal ID (from extattrs_all) is missing or invalid.",
			)
			return
		}
	...
	if diags.HasError() {
		resp.Diagnostics.Append(diags...)
		resp.Diagnostics.AddError("Client Error", "Error while reading Vlan due to inherited Extensible attributes")
		return
	}
...
func (r *VlanResource) ReadByExtAttrs(ctx context.Context, data *VlanModel, resp *resource.ReadResponse) bool {
	var diags diag.Diagnostics
	...
	apiRes, _, err := r.client.IPAMAPI.
		...
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Vlan by extattrs, got error: %s", err))
		return true
	}

	results := apiRes.ListVlanResponseObject.GetResult()
...
func (r *VlanResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if diags.HasError() {
		resp.Diagnostics.Append(diags...)
		resp.Diagnostics.AddError("Client Error", "Error while updating Vlan due to inherited Extensible attributes")
		return
	}

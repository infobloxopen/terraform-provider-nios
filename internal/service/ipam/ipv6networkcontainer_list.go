...
var _ list.ListResource = &Ipv6networkcontainerList{}
var _ list.ListResourceWithConfigure = &Ipv6networkcontainerList{}

func NewIpv6networkcontainerList() list.ListResource {
	return &Ipv6networkcontainerList{}
}

// Ipv6networkcontainerList defines the List implementation.
type Ipv6networkcontainerList struct {
	client *niosclient.APIClient
}

func (l *Ipv6networkcontainerList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_ipv6network_container"
}

func (l *Ipv6networkcontainerList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type Ipv6networkcontainerListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *Ipv6networkcontainerList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing IPAM IPv6 Network Container",
...
func (l *Ipv6networkcontainerList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data Ipv6networkcontainerListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]ipam.Ipv6networkcontainer, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Ipv6networkcontainer, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenIpv6networkcontainer(ctx, &item, &result.Diagnostics)

...
var _ list.ListResource = &NetworktemplateList{}
var _ list.ListResourceWithConfigure = &NetworktemplateList{}

func NewNetworktemplateList() list.ListResource {
	return &NetworktemplateList{}
}

// NetworktemplateList defines the List implementation.
type NetworktemplateList struct {
	client *niosclient.APIClient
}

func (l *NetworktemplateList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_networktemplate"
}

func (l *NetworktemplateList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type NetworktemplateListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *NetworktemplateList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing IPAM Network Templates.",
...
func (l *NetworktemplateList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data NetworktemplateListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]ipam.Networktemplate, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Networktemplate, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenNetworktemplate(ctx, &item, &result.Diagnostics)

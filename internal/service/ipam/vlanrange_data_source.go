...
var _ datasource.DataSource = &VlanrangeDataSource{}

func NewVlanrangeDataSource() datasource.DataSource {
	return &VlanrangeDataSource{}
}

// VlanrangeDataSource defines the data source implementation.
type VlanrangeDataSource struct {
	client *niosclient.APIClient
}

func (d *VlanrangeDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_vlanrange"
}

type VlanrangeModelWithFilter struct {
...
func (m *VlanrangeModelWithFilter) FlattenResults(ctx context.Context, from []ipam.Vlanrange, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, VlanrangeAttrTypes, diags, FlattenVlanrange)
}

func (d *VlanrangeDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Vlan Ranges.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(VlanrangeResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *VlanrangeDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *VlanrangeDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data VlanrangeModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]ipam.Vlanrange, string, error) {
			...
			res := apiRes.ListVlanrangeResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListVlanrangeResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &NetworkviewDataSource{}

func NewNetworkviewDataSource() datasource.DataSource {
	return &NetworkviewDataSource{}
}

// NetworkviewDataSource defines the data source implementation.
type NetworkviewDataSource struct {
	client *niosclient.APIClient
}

func (d *NetworkviewDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_network_view"
}

type NetworkviewModelWithFilter struct {
...
func (m *NetworkviewModelWithFilter) FlattenResults(ctx context.Context, from []ipam.Networkview, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, NetworkviewAttrTypes, diags, FlattenNetworkview)
}

func (d *NetworkviewDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Network Views.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(NetworkviewResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *NetworkviewDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NetworkviewDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data NetworkviewModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]ipam.Networkview, string, error) {

...
var _ list.ListResource = &VlanList{}
var _ list.ListResourceWithConfigure = &VlanList{}

func NewVlanList() list.ListResource {
	return &VlanList{}
}

// VlanList defines the List implementation.
type VlanList struct {
	client *niosclient.APIClient
}

func (l *VlanList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_vlan"
}

func (l *VlanList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type VlanListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *VlanList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing IPAM VLANs.",
...
func (l *VlanList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data VlanListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]ipam.Vlan, string, error) {
			...
			res := apiRes.ListVlanResponseObject.GetResult()
			...
			additionalProperties := apiRes.ListVlanResponseObject.AdditionalProperties
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Vlan, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}

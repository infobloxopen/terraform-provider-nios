...
var _ list.ListResource = &SuperhostList{}
var _ list.ListResourceWithConfigure = &SuperhostList{}

func NewSuperhostList() list.ListResource {
	return &SuperhostList{}
}

// SuperhostList defines the List implementation.
type SuperhostList struct {
	client *niosclient.APIClient
}

func (l *SuperhostList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_superhost"
}

func (l *SuperhostList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type SuperhostListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *SuperhostList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing IPAM Superhosts.",
...
func (l *SuperhostList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data SuperhostListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]ipam.Superhost, string, error) {
			...
			if apiRes == nil || apiRes.ListSuperhostResponseObject == nil {
				return nil, "", nil
			}
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Superhost, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenSuperhost(ctx, &item, &result.Diagnostics)

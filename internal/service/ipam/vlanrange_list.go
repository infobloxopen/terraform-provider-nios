...
var _ list.ListResource = &VlanrangeList{}
var _ list.ListResourceWithConfigure = &VlanrangeList{}

func NewVlanrangeList() list.ListResource {
	return &VlanrangeList{}
}

// VlanrangeList defines the List implementation.
type VlanrangeList struct {
	client *niosclient.APIClient
}

func (l *VlanrangeList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_vlanrange"
}

func (l *VlanrangeList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type VlanrangeListModel struct {
...
func (l *VlanrangeList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data VlanrangeListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]ipam.Vlanrange, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Vlanrange, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenVlanrange(ctx, &item, &result.Diagnostics)

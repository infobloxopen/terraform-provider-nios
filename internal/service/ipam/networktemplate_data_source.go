...
var _ datasource.DataSource = &NetworktemplateDataSource{}

func NewNetworktemplateDataSource() datasource.DataSource {
	return &NetworktemplateDataSource{}
}

// NetworktemplateDataSource defines the data source implementation.
type NetworktemplateDataSource struct {
	client *niosclient.APIClient
}

func (d *NetworktemplateDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_networktemplate"
}

type NetworktemplateModelWithFilter struct {
...
func (m *NetworktemplateModelWithFilter) FlattenResults(ctx context.Context, from []ipam.Networktemplate, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, NetworktemplateAttrTypes, diags, FlattenNetworktemplate)
}

func (d *NetworktemplateDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Networktemplate objects.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(NetworktemplateResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *NetworktemplateDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NetworktemplateDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data NetworktemplateModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]ipam.Networktemplate, string, error) {
			...
			res := apiRes.ListNetworktemplateResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListNetworktemplateResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &VlanviewDataSource{}

func NewVlanviewDataSource() datasource.DataSource {
	return &VlanviewDataSource{}
}

// VlanviewDataSource defines the data source implementation.
type VlanviewDataSource struct {
	client *niosclient.APIClient
}

func (d *VlanviewDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_vlanview"
}

type VlanviewModelWithFilter struct {
...
func (m *VlanviewModelWithFilter) FlattenResults(ctx context.Context, from []ipam.Vlanview, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, VlanviewAttrTypes, diags, FlattenVlanview)
}

func (d *VlanviewDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing VLAN Views.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(VlanviewResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *VlanviewDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *VlanviewDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data VlanviewModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]ipam.Vlanview, string, error) {
			...
			res := apiRes.ListVlanviewResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListVlanviewResponseObject.AdditionalProperties

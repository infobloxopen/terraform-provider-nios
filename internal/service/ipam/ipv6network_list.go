...
var _ list.ListResource = &Ipv6networkList{}
var _ list.ListResourceWithConfigure = &Ipv6networkList{}

func NewIpv6networkList() list.ListResource {
	return &Ipv6networkList{}
}

// Ipv6networkList defines the List implementation.
type Ipv6networkList struct {
	client *niosclient.APIClient
}

func (l *Ipv6networkList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_ipv6network"
}

func (l *Ipv6networkList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type Ipv6networkListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *Ipv6networkList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing IPAM IPv6 Network.",
...
func (l *Ipv6networkList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data Ipv6networkListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]ipam.Ipv6network, string, error) {
			...
			res := apiRes.ListIpv6networkResponseObject.GetResult()
			...
			additionalProperties := apiRes.ListIpv6networkResponseObject.AdditionalProperties
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Ipv6network, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenIpv6network(ctx, &item, &result.Diagnostics)

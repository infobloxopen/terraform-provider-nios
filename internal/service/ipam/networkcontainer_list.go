...
var _ list.ListResource = &NetworkcontainerList{}
var _ list.ListResourceWithConfigure = &NetworkcontainerList{}

func NewNetworkcontainerList() list.ListResource {
	return &NetworkcontainerList{}
}

// NetworkcontainerList defines the List implementation.
type NetworkcontainerList struct {
	client *niosclient.APIClient
}

func (l *NetworkcontainerList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_network_container"
}

func (l *NetworkcontainerList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type NetworkcontainerListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *NetworkcontainerList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing IPAM Network Container.",
...
func (l *NetworkcontainerList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]ipam.Networkcontainer, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Networkcontainer, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenNetworkcontainer(ctx, &item, &result.Diagnostics)

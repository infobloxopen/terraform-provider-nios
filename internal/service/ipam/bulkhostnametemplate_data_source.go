...
var _ datasource.DataSource = &BulkhostnametemplateDataSource{}

func NewBulkhostnametemplateDataSource() datasource.DataSource {
	return &BulkhostnametemplateDataSource{}
}

// BulkhostnametemplateDataSource defines the data source implementation.
type BulkhostnametemplateDataSource struct {
	client *niosclient.APIClient
}

func (d *BulkhostnametemplateDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (m *BulkhostnametemplateModelWithFilter) FlattenResults(ctx context.Context, from []ipam.Bulkhostnametemplate, diags *diag.Diagnostics) {
	...
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, BulkhostnametemplateAttrTypes, diags, FlattenBulkhostnametemplate)
}

func (d *BulkhostnametemplateDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The bulk hostname template data source allows you to retrieve and filter bulk hostname template objects from Infoblox NIOS. Use filters to narrow down results by specific attributes.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(BulkhostnametemplateResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *BulkhostnametemplateDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *BulkhostnametemplateDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data BulkhostnametemplateModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]ipam.Bulkhostnametemplate, string, error) {

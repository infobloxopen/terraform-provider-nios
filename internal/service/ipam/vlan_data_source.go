...
type VlanModelWithFilter struct {
...
func (m *VlanModelWithFilter) FlattenResults(ctx context.Context, from []ipam.Vlan, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, VlanAttrTypes, diags, FlattenVlan)
}

func (d *VlanDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Vlan objects.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(VlanResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *VlanDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]ipam.Vlan, string, error) {

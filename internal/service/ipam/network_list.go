...
var _ list.ListResource = &NetworkList{}
var _ list.ListResourceWithConfigure = &NetworkList{}

func NewNetworkList() list.ListResource {
	return &NetworkList{}
}

// NetworkList defines the List implementation.
type NetworkList struct {
	client *niosclient.APIClient
}

func (l *NetworkList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_network"
}

func (l *NetworkList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (l *NetworkList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing IPAM Network.",
...
func (l *NetworkList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]ipam.Network, string, error) {
			...
			res := apiRes.ListNetworkResponseObject.GetResult()
			...
			additionalProperties := apiRes.ListNetworkResponseObject.AdditionalProperties
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Network, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenNetwork(ctx, &item, &result.Diagnostics)

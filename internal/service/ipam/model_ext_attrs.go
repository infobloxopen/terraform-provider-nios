...
func ExpandExtAttrs(ctx context.Context, extattrs types.Map, diags *diag.Diagnostics) *map[string]ipam.ExtAttrs {
	...
	for key, valStr := range extAttrsMap {
		...
		result[key] = ipam.ExtAttrs{Value: parsedValue}
	}
	return &result
}

func FlattenExtAttrs(ctx context.Context, planExtAttrs types.Map, extattrs *map[string]ipam.ExtAttrs, diags *diag.Diagnostics) types.Map {

...
var _ datasource.DataSource = &SuperhostDataSource{}

func NewSuperhostDataSource() datasource.DataSource {
	return &SuperhostDataSource{}
}

// SuperhostDataSource defines the data source implementation.
type SuperhostDataSource struct {
	client *niosclient.APIClient
}

func (d *SuperhostDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "ipam_superhost"
}

type SuperhostModelWithFilter struct {
...
func (m *SuperhostModelWithFilter) FlattenResults(ctx context.Context, from []ipam.Superhost, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, SuperhostAttrTypes, diags, FlattenSuperhost)
}

func (d *SuperhostDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Superhost objects.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(SuperhostResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *SuperhostDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *SuperhostDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data SuperhostModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]ipam.Superhost, string, error) {
			...
			res := apiRes.ListSuperhostResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListSuperhostResponseObject.AdditionalProperties

...
type RecordSvcbCloudInfoModel struct {
...
var RecordSvcbCloudInfoAttrTypes = map[string]attr.Type{
	"delegated_member": types.ObjectType{AttrTypes: RecordsvcbcloudinfoDelegatedMemberAttrTypes},
...
var RecordSvcbCloudInfoResourceSchemaAttributes = map[string]schema.Attribute{
	"delegated_member": schema.SingleNestedAttribute{
		Attributes: RecordsvcbcloudinfoDelegatedMemberResourceSchemaAttributes,
		Optional:   true,
	},
...
func ExpandRecordSvcbCloudInfo(ctx context.Context, o types.Object, diags *diag.Diagnostics) *dns.RecordSvcbCloudInfo {
	if o.IsNull() || o.IsUnknown() {
		return nil
	}
	var m RecordSvcbCloudInfoModel
...
func (m *RecordSvcbCloudInfoModel) Expand(ctx context.Context, diags *diag.Diagnostics) *dns.RecordSvcbCloudInfo {
	if m == nil {
		return nil
	}
	to := &dns.RecordSvcbCloudInfo{
		DelegatedMember: ExpandRecordsvcbcloudinfoDelegatedMember(ctx, m.DelegatedMember, diags),
	}
	return to
}

func FlattenRecordSvcbCloudInfo(ctx context.Context, from *dns.RecordSvcbCloudInfo, diags *diag.Diagnostics) types.Object {
	if from == nil {
		return types.ObjectNull(RecordSvcbCloudInfoAttrTypes)
	}
	m := RecordSvcbCloudInfoModel{}
	m.Flatten(ctx, from, diags)
	t, d := types.ObjectValueFrom(ctx, RecordSvcbCloudInfoAttrTypes, m)
	diags.Append(d...)
	return t
}

func (m *RecordSvcbCloudInfoModel) Flatten(ctx context.Context, from *dns.RecordSvcbCloudInfo, diags *diag.Diagnostics) {
	if from == nil {
		return
	}
	if m == nil {
	...
	m.DelegatedMember = FlattenRecordsvcbcloudinfoDelegatedMember(ctx, from.DelegatedMember, diags)

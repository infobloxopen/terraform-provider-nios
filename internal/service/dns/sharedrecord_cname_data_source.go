...
var _ datasource.DataSource = &SharedrecordCnameDataSource{}

func NewSharedrecordCnameDataSource() datasource.DataSource {
	return &SharedrecordCnameDataSource{}
}

// SharedrecordCnameDataSource defines the data source implementation.
type SharedrecordCnameDataSource struct {
	client *niosclient.APIClient
}

func (d *SharedrecordCnameDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_sharedrecord_cname"
}

type SharedrecordCnameModelWithFilter struct {
...
func (m *SharedrecordCnameModelWithFilter) FlattenResults(ctx context.Context, from []dns.SharedrecordCname, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, SharedrecordCnameAttrTypes, diags, FlattenSharedrecordCname)
}

func (d *SharedrecordCnameDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DNS Shared CNAME Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(SharedrecordCnameResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *SharedrecordCnameDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *SharedrecordCnameDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data SharedrecordCnameModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dns.SharedrecordCname, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read SharedrecordCname by extattrs, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListSharedrecordCnameResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListSharedrecordCnameResponseObject.AdditionalProperties

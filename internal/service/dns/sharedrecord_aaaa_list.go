...
var _ list.ListResource = &SharedrecordAaaaList{}
var _ list.ListResourceWithConfigure = &SharedrecordAaaaList{}

func NewSharedrecordAaaaList() list.ListResource {
	return &SharedrecordAaaaList{}
}

// SharedrecordAaaaList defines the List implementation.
type SharedrecordAaaaList struct {
	client *niosclient.APIClient
}

func (l *SharedrecordAaaaList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_sharedrecord_aaaa"
}

func (l *SharedrecordAaaaList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type SharedrecordAaaaListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *SharedrecordAaaaList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Shared AAAA Records.",
...
func (l *SharedrecordAaaaList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data SharedrecordAaaaListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.SharedrecordAaaa, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list SharedrecordAaaa, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenSharedrecordAaaa(ctx, &item, &result.Diagnostics)

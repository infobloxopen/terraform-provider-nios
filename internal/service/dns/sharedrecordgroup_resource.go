...
func (r *SharedrecordgroupResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	var diags diag.Diagnostics
	var data SharedrecordgroupModel
	var goClientData dns.Sharedrecordgroup
	...
	updateRes, _, err := r.client.DNSAPI.
		...
		Sharedrecordgroup(goClientData).
		ReturnFieldsPlus(readableAttributesForSharedrecordgroup).
		ReturnAsObject(1).
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Import Failed", fmt.Sprintf("Unable to update Sharedrecordgroup for import, got error: %s", err))
		return
	}

	res := updateRes.UpdateSharedrecordgroupResponseAsObject.GetResult()

	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrsAll, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update Sharedrecordgroup due inherited Extensible attributes for import, got error: %s", diags))
		return
	}

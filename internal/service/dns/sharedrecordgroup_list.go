...
var _ list.ListResource = &SharedrecordgroupList{}
var _ list.ListResourceWithConfigure = &SharedrecordgroupList{}

func NewSharedrecordgroupList() list.ListResource {
	return &SharedrecordgroupList{}
}

// SharedrecordgroupList defines the List implementation.
type SharedrecordgroupList struct {
	client *niosclient.APIClient
}

func (l *SharedrecordgroupList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_sharedrecordgroup"
}

func (l *SharedrecordgroupList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type SharedrecordgroupListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *SharedrecordgroupList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Shared Record Groups.",
...
func (l *SharedrecordgroupList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data SharedrecordgroupListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.Sharedrecordgroup, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Sharedrecordgroup, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenSharedrecordgroup(ctx, &item, &result.Diagnostics)

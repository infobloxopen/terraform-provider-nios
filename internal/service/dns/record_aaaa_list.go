...
var _ list.ListResource = &RecordAaaaList{}
var _ list.ListResourceWithConfigure = &RecordAaaaList{}

func NewRecordAaaaList() list.ListResource {
	return &RecordAaaaList{}
}

// RecordAaaaList defines the List implementation.
type RecordAaaaList struct {
	client *niosclient.APIClient
}

func (l *RecordAaaaList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_record_aaaa"
}

func (l *RecordAaaaList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type RecordAaaaListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *RecordAaaaList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS AAAA records.",
...
func (l *RecordAaaaList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.RecordAaaa, string, error) {
			...
			additionalProperties := apiRes.ListRecordAaaaResponseObject.AdditionalProperties
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordAaaa, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenRecordAaaa(ctx, &item, &result.Diagnostics)

...
var _ list.ListResource = &ZoneForwardList{}
var _ list.ListResourceWithConfigure = &ZoneForwardList{}

func NewZoneForwardList() list.ListResource {
	return &ZoneForwardList{}
}

// ZoneForwardList defines the List implementation.
type ZoneForwardList struct {
	client *niosclient.APIClient
}

func (l *ZoneForwardList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_zone_forward"
}

func (l *ZoneForwardList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type ZoneForwardListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *ZoneForwardList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Forward Zones.",
...
func (l *ZoneForwardList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data ZoneForwardListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.ZoneForward, string, error) {
			...
			additionalProperties := apiRes.ListZoneForwardResponseObject.AdditionalProperties
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list ZoneForward, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenZoneForward(ctx, &item, &result.Diagnostics)

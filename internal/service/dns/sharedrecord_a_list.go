...
var _ list.ListResource = &SharedrecordAList{}
var _ list.ListResourceWithConfigure = &SharedrecordAList{}

func NewSharedrecordAList() list.ListResource {
	return &SharedrecordAList{}
}

// SharedrecordAList defines the List implementation.
type SharedrecordAList struct {
	client *niosclient.APIClient
}

func (l *SharedrecordAList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_sharedrecord_a"
}

func (l *SharedrecordAList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type SharedrecordAListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *SharedrecordAList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Shared A Records.",
...
func (l *SharedrecordAList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data SharedrecordAListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.SharedrecordA, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list SharedrecordA, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenSharedrecordA(ctx, &item, &result.Diagnostics)

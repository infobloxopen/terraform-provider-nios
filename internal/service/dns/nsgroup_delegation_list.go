...
var _ list.ListResource = &NsgroupDelegationList{}
var _ list.ListResourceWithConfigure = &NsgroupDelegationList{}

func NewNsgroupDelegationList() list.ListResource {
	return &NsgroupDelegationList{}
}

// NsgroupDelegationList defines the List implementation.
type NsgroupDelegationList struct {
	client *niosclient.APIClient
}

func (l *NsgroupDelegationList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_nsgroup_delegation"
}

func (l *NsgroupDelegationList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type NsgroupDelegationListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *NsgroupDelegationList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS NS Group Delegations.",
...
func (l *NsgroupDelegationList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data NsgroupDelegationListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.NsgroupDelegation, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list NsgroupDelegation, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenNsgroupDelegation(ctx, &item, &result.Diagnostics)

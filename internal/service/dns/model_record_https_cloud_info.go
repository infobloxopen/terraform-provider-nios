...
type RecordHttpsCloudInfoModel struct {
...
var RecordHttpsCloudInfoAttrTypes = map[string]attr.Type{
	"delegated_member": types.ObjectType{AttrTypes: RecordhttpscloudinfoDelegatedMemberAttrTypes},
...
var RecordHttpsCloudInfoResourceSchemaAttributes = map[string]schema.Attribute{
	"delegated_member": schema.SingleNestedAttribute{
		Attributes: RecordhttpscloudinfoDelegatedMemberResourceSchemaAttributes,
		Optional:   true,
	},
...
func ExpandRecordHttpsCloudInfo(ctx context.Context, o types.Object, diags *diag.Diagnostics) *dns.RecordHttpsCloudInfo {
	if o.IsNull() || o.IsUnknown() {
		return nil
	}
	var m RecordHttpsCloudInfoModel
...
func (m *RecordHttpsCloudInfoModel) Expand(ctx context.Context, diags *diag.Diagnostics) *dns.RecordHttpsCloudInfo {
	if m == nil {
		return nil
	}
	to := &dns.RecordHttpsCloudInfo{
		DelegatedMember: ExpandRecordhttpscloudinfoDelegatedMember(ctx, m.DelegatedMember, diags),
	}
	return to
}

func FlattenRecordHttpsCloudInfo(ctx context.Context, from *dns.RecordHttpsCloudInfo, diags *diag.Diagnostics) types.Object {
	if from == nil {
		return types.ObjectNull(RecordHttpsCloudInfoAttrTypes)
	}
	m := RecordHttpsCloudInfoModel{}
	m.Flatten(ctx, from, diags)
	t, d := types.ObjectValueFrom(ctx, RecordHttpsCloudInfoAttrTypes, m)
	diags.Append(d...)
	return t
}

func (m *RecordHttpsCloudInfoModel) Flatten(ctx context.Context, from *dns.RecordHttpsCloudInfo, diags *diag.Diagnostics) {
	if from == nil {
		return
	}
	if m == nil {
	...
	m.DelegatedMember = FlattenRecordhttpscloudinfoDelegatedMember(ctx, from.DelegatedMember, diags)

...
var _ list.ListResource = &RecordCnameList{}
var _ list.ListResourceWithConfigure = &RecordCnameList{}

func NewRecordCnameList() list.ListResource {
	return &RecordCnameList{}
}

// RecordCnameList defines the List implementation.
type RecordCnameList struct {
	client *niosclient.APIClient
}

func (l *RecordCnameList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_record_cname"
}

func (l *RecordCnameList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type RecordCnameListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *RecordCnameList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS CNAME Records.",
...
func (l *RecordCnameList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.RecordCname, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordCname, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenRecordCname(ctx, &item, &result.Diagnostics)

...
var _ list.ListResource = &ZoneDelegatedList{}
var _ list.ListResourceWithConfigure = &ZoneDelegatedList{}

func NewZoneDelegatedList() list.ListResource {
	return &ZoneDelegatedList{}
}

// ZoneDelegatedList defines the List implementation.
type ZoneDelegatedList struct {
	client *niosclient.APIClient
}

func (l *ZoneDelegatedList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_zone_delegated"
}

func (l *ZoneDelegatedList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type ZoneDelegatedListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *ZoneDelegatedList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Delegated Zones.",
...
func (l *ZoneDelegatedList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data ZoneDelegatedListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.ZoneDelegated, string, error) {
			...
			res := apiRes.ListZoneDelegatedResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListZoneDelegatedResponseObject.AdditionalProperties
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list ZoneDelegated, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenZoneDelegated(ctx, &item, &result.Diagnostics)

...
var _ datasource.DataSource = &NsgroupDelegationDataSource{}

func NewNsgroupDelegationDataSource() datasource.DataSource {
	return &NsgroupDelegationDataSource{}
}

// NsgroupDelegationDataSource defines the data source implementation.
type NsgroupDelegationDataSource struct {
	client *niosclient.APIClient
}

func (d *NsgroupDelegationDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_nsgroup_delegation"
}

type NsgroupDelegationModelWithFilter struct {
...
func (m *NsgroupDelegationModelWithFilter) FlattenResults(ctx context.Context, from []dns.NsgroupDelegation, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, NsgroupDelegationAttrTypes, diags, FlattenNsgroupDelegation)
}

func (d *NsgroupDelegationDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing NS Group delegations.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(NsgroupDelegationResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *NsgroupDelegationDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NsgroupDelegationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data NsgroupDelegationModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dns.NsgroupDelegation, string, error) {
			...
			res := apiRes.ListNsgroupDelegationResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListNsgroupDelegationResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &SharedrecordgroupDataSource{}

func NewSharedrecordgroupDataSource() datasource.DataSource {
	return &SharedrecordgroupDataSource{}
}

// SharedrecordgroupDataSource defines the data source implementation.
type SharedrecordgroupDataSource struct {
	client *niosclient.APIClient
}

func (d *SharedrecordgroupDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_sharedrecordgroup"
}

type SharedrecordgroupModelWithFilter struct {
...
func (m *SharedrecordgroupModelWithFilter) FlattenResults(ctx context.Context, from []dns.Sharedrecordgroup, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, SharedrecordgroupAttrTypes, diags, FlattenSharedrecordgroup)
}

func (d *SharedrecordgroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Shared Record Groups.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(SharedrecordgroupResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *SharedrecordgroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *SharedrecordgroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data SharedrecordgroupModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dns.Sharedrecordgroup, string, error) {
			...
			res := apiRes.ListSharedrecordgroupResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListSharedrecordgroupResponseObject.AdditionalProperties

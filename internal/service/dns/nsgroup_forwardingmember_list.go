...
var _ list.ListResource = &NsgroupForwardingmemberList{}
var _ list.ListResourceWithConfigure = &NsgroupForwardingmemberList{}

func NewNsgroupForwardingmemberList() list.ListResource {
	return &NsgroupForwardingmemberList{}
}

// NsgroupForwardingmemberList defines the List implementation.
type NsgroupForwardingmemberList struct {
	client *niosclient.APIClient
}

func (l *NsgroupForwardingmemberList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_nsgroup_forwardingmember"
}

func (l *NsgroupForwardingmemberList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type NsgroupForwardingmemberListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *NsgroupForwardingmemberList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS NS Group Forwarding Members.",
...
func (l *NsgroupForwardingmemberList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data NsgroupForwardingmemberListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.NsgroupForwardingmember, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list NsgroupForwardingmember, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenNsgroupForwardingmember(ctx, &item, &result.Diagnostics)

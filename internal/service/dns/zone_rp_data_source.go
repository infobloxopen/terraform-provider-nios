...
var _ datasource.DataSource = &ZoneRpDataSource{}

func NewZoneRpDataSource() datasource.DataSource {
	return &ZoneRpDataSource{}
}

// ZoneRpDataSource defines the data source implementation.
type ZoneRpDataSource struct {
	client *niosclient.APIClient
}

func (d *ZoneRpDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_zone_rp"
}

type ZoneRpModelWithFilter struct {
...
func (m *ZoneRpModelWithFilter) FlattenResults(ctx context.Context, from []dns.ZoneRp, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, ZoneRpAttrTypes, diags, FlattenZoneRp)
}

func (d *ZoneRpDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DNS RPZs (Response Policy Zones).",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(ZoneRpResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *ZoneRpDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ZoneRpDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data ZoneRpModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dns.ZoneRp, string, error) {
			...
			res := apiRes.ListZoneRpResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListZoneRpResponseObject.AdditionalProperties

...
var _ list.ListResource = &SharedrecordCnameList{}
var _ list.ListResourceWithConfigure = &SharedrecordCnameList{}

func NewSharedrecordCnameList() list.ListResource {
	return &SharedrecordCnameList{}
}

// SharedrecordCnameList defines the List implementation.
type SharedrecordCnameList struct {
	client *niosclient.APIClient
}

func (l *SharedrecordCnameList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_sharedrecord_cname"
}

func (l *SharedrecordCnameList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (l *SharedrecordCnameList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Shared CNAME Records.",
...
func (l *SharedrecordCnameList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.SharedrecordCname, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list SharedrecordCname, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenSharedrecordCname(ctx, &item, &result.Diagnostics)

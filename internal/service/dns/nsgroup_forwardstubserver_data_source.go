...
// NsgroupForwardstubserverDataSource defines the data source implementation.
type NsgroupForwardstubserverDataSource struct {
	client *niosclient.APIClient
}

func (d *NsgroupForwardstubserverDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_nsgroup_forwardstubserver"
}

type NsgroupForwardstubserverModelWithFilter struct {
...
func (m *NsgroupForwardstubserverModelWithFilter) FlattenResults(ctx context.Context, from []dns.NsgroupForwardstubserver, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, NsgroupForwardstubserverAttrTypes, diags, FlattenNsgroupForwardstubserver)
}

func (d *NsgroupForwardstubserverDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing NS Group Forward Stub Servers.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(NsgroupForwardstubserverResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *NsgroupForwardstubserverDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NsgroupForwardstubserverDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data NsgroupForwardstubserverModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dns.NsgroupForwardstubserver, string, error) {

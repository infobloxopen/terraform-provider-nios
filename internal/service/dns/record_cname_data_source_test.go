...
func TestAccRecordCnameDataSource_Filters(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccRecordCnameDataSourceConfigFilters(),
...
func TestAccRecordCnameDataSource_ExtAttrFilters(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccRecordCnameDataSourceConfigExtAttrFilters(, "value1"),
				Check: resource.ComposeTestCheckFunc(
					append([]resource.TestCheckFunc{
							testAccCheckRecordCnameExists(context.Background(), resourceName, &v),
						}, testAccCheckRecordCnameResourceAttrPair(resourceName, dataSourceName)...)...,
...
func testAccRecordCnameDataSourceConfigFilters() string {
	return fmt.Sprintf(`
...
data "nios_dns_record_cname" "test" {
  filters = {
	 = nios_dns_record_cname.test.
  }
}
`)
}

func testAccRecordCnameDataSourceConfigExtAttrFilters(extAttrsValue string) string {
	return fmt.Sprintf(`
resource "nios_dns_record_cname" "test" {
  extattrs = {
    Site = %q

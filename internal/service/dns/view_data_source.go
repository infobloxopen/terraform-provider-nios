...
type ViewModelWithFilter struct {
...
func (m *ViewModelWithFilter) FlattenResults(ctx context.Context, from []dns.View, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, ViewAttrTypes, diags, FlattenView)
}

func (d *ViewDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about exisiting DNS Views",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(ViewResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *ViewDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dns.View, string, error) {

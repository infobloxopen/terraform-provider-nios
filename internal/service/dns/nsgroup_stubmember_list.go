...
var _ list.ListResource = &NsgroupStubmemberList{}
var _ list.ListResourceWithConfigure = &NsgroupStubmemberList{}

func NewNsgroupStubmemberList() list.ListResource {
	return &NsgroupStubmemberList{}
}

// NsgroupStubmemberList defines the List implementation.
type NsgroupStubmemberList struct {
	client *niosclient.APIClient
}

func (l *NsgroupStubmemberList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_nsgroup_stubmember"
}

func (l *NsgroupStubmemberList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type NsgroupStubmemberListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *NsgroupStubmemberList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS NS Group Stub Members.",
...
func (l *NsgroupStubmemberList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data NsgroupStubmemberListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.NsgroupStubmember, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list NsgroupStubmember, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenNsgroupStubmember(ctx, &item, &result.Diagnostics)

...
var _ list.ListResource = &RecordAliasList{}
var _ list.ListResourceWithConfigure = &RecordAliasList{}

func NewRecordAliasList() list.ListResource {
	return &RecordAliasList{}
}

// RecordAliasList defines the List implementation.
type RecordAliasList struct {
	client *niosclient.APIClient
}

func (l *RecordAliasList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_record_alias"
}

func (l *RecordAliasList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type RecordAliasListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *RecordAliasList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Alias records.",
...
func (l *RecordAliasList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.RecordAlias, string, error) {
			...
			res := apiRes.ListRecordAliasResponseObject.GetResult()
			...
			additionalProperties := apiRes.ListRecordAliasResponseObject.AdditionalProperties
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordAlias, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenRecordAlias(ctx, &item, &result.Diagnostics)

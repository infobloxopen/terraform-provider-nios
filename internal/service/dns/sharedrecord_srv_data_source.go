...
var _ datasource.DataSource = &SharedrecordSrvDataSource{}

func NewSharedrecordSrvDataSource() datasource.DataSource {
	return &SharedrecordSrvDataSource{}
}

// SharedrecordSrvDataSource defines the data source implementation.
type SharedrecordSrvDataSource struct {
	client *niosclient.APIClient
}

func (d *SharedrecordSrvDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_sharedrecord_srv"
}

type SharedrecordSrvModelWithFilter struct {
...
func (m *SharedrecordSrvModelWithFilter) FlattenResults(ctx context.Context, from []dns.SharedrecordSrv, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, SharedrecordSrvAttrTypes, diags, FlattenSharedrecordSrv)
}

func (d *SharedrecordSrvDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DNS Shared SRV Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(SharedrecordSrvResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *SharedrecordSrvDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *SharedrecordSrvDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data SharedrecordSrvModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dns.SharedrecordSrv, string, error) {
			...
			res := apiRes.ListSharedrecordSrvResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListSharedrecordSrvResponseObject.AdditionalProperties

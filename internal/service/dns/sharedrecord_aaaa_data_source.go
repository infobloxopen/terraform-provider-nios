...
var _ datasource.DataSource = &SharedrecordAaaaDataSource{}

func NewSharedrecordAaaaDataSource() datasource.DataSource {
	return &SharedrecordAaaaDataSource{}
}

// SharedrecordAaaaDataSource defines the data source implementation.
type SharedrecordAaaaDataSource struct {
	client *niosclient.APIClient
}

func (d *SharedrecordAaaaDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_sharedrecord_aaaa"
}

type SharedrecordAaaaModelWithFilter struct {
...
func (m *SharedrecordAaaaModelWithFilter) FlattenResults(ctx context.Context, from []dns.SharedrecordAaaa, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, SharedrecordAaaaAttrTypes, diags, FlattenSharedrecordAaaa)
}

func (d *SharedrecordAaaaDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DNS Shared AAAA Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(SharedrecordAaaaResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *SharedrecordAaaaDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *SharedrecordAaaaDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data SharedrecordAaaaModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dns.SharedrecordAaaa, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read SharedrecordAaaa by extattrs, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListSharedrecordAaaaResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListSharedrecordAaaaResponseObject.AdditionalProperties

...
var _ list.ListResource = &RecordTxtList{}
var _ list.ListResourceWithConfigure = &RecordTxtList{}

func NewRecordTxtList() list.ListResource {
	return &RecordTxtList{}
}

// RecordTxtList defines the List implementation.
type RecordTxtList struct {
	client *niosclient.APIClient
}

func (l *RecordTxtList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_record_txt"
}

func (l *RecordTxtList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type RecordTxtListModel struct {
...
func (l *RecordTxtList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.RecordTxt, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordTxt, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}

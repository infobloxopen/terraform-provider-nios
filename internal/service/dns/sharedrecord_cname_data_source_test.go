...
func TestAccSharedrecordCnameDataSource_Filters(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSharedrecordCnameDataSourceConfigFilters(),
...
func TestAccSharedrecordCnameDataSource_ExtAttrFilters(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSharedrecordCnameDataSourceConfigExtAttrFilters(, acctest.RandomName()),
				Check: resource.ComposeTestCheckFunc(
					append([]resource.TestCheckFunc{
							testAccCheckSharedrecordCnameExists(context.Background(), resourceName, &v),
						}, testAccCheckSharedrecordCnameResourceAttrPair(resourceName, dataSourceName)...)...,
...
func testAccSharedrecordCnameDataSourceConfigFilters() string {
	return fmt.Sprintf(`
...
data "nios_dns_sharedrecord_cname" "test" {
  filters = {
	 = nios_dns_sharedrecord_cname.test.
  }
}
`)
}

func testAccSharedrecordCnameDataSourceConfigExtAttrFilters(extAttrsValue string) string {
	return fmt.Sprintf(`
resource "nios_dns_sharedrecord_cname" "test" {
  extattrs = {
    Site = %q

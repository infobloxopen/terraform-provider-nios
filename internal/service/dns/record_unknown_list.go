...
var _ list.ListResource = &RecordUnknownList{}
var _ list.ListResourceWithConfigure = &RecordUnknownList{}

func NewRecordUnknownList() list.ListResource {
	return &RecordUnknownList{}
}

// RecordUnknownList defines the List implementation.
type RecordUnknownList struct {
	client *niosclient.APIClient
}

func (l *RecordUnknownList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_record_unknown"
}

func (l *RecordUnknownList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type RecordUnknownListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *RecordUnknownList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Unknown Records.",
...
func (l *RecordUnknownList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data RecordUnknownListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.RecordUnknown, string, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordUnknown, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenRecordUnknown(ctx, &item, &result.Diagnostics)

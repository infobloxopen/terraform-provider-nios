...
var _ list.ListResource = &NsgroupForwardstubserverList{}
var _ list.ListResourceWithConfigure = &NsgroupForwardstubserverList{}

func NewNsgroupForwardstubserverList() list.ListResource {
	return &NsgroupForwardstubserverList{}
}

// NsgroupForwardstubserverList defines the List implementation.
type NsgroupForwardstubserverList struct {
	client *niosclient.APIClient
}

func (l *NsgroupForwardstubserverList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_nsgroup_forwardstubserver"
}

func (l *NsgroupForwardstubserverList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type NsgroupForwardstubserverListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *NsgroupForwardstubserverList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS NS Group Forward Stub Servers.",
...
func (l *NsgroupForwardstubserverList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data NsgroupForwardstubserverListModel
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.NsgroupForwardstubserver, string, error) {
			...
			res := apiRes.ListNsgroupForwardstubserverResponseObject.GetResult()
			...
			additionalProperties := apiRes.ListNsgroupForwardstubserverResponseObject.AdditionalProperties
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list NsgroupForwardstubserver, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenNsgroupForwardstubserver(ctx, &item, &result.Diagnostics)

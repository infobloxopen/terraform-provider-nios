...
var _ list.ListResource = &ViewList{}
var _ list.ListResourceWithConfigure = &ViewList{}

func NewViewList() list.ListResource {
	return &ViewList{}
}

// ViewList defines the List implementation.
type ViewList struct {
	client *niosclient.APIClient
}

func (l *ViewList) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dns_view"
}

func (l *ViewList) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
type ViewListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"extattrfilters"`
}

func (l *ViewList) ListResourceConfigSchema(ctx context.Context, req list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Query existing DNS Views.",
...
func (l *ViewList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResultsPerPage int32) ([]dns.View, string, error) {
			...
			res := apiRes.ListViewResponseObject.GetResult()
			...
			additionalProperties := apiRes.ListViewResponseObject.AdditionalProperties
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list View, got error: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
	...
	stream.Results = func(push func(list.ListResult) bool) {
		for _, item := range allResults {
			...
			if req.IncludeResource {
				...
				result1 := FlattenView(ctx, &item, &result.Diagnostics)

...
func (r *PxgridEndpointResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while create PxgridEndpoint due inherited Extensible attributes, got error: %s", err))
		return
	}
...
func (r *PxgridEndpointResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrs, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while reading PxgridEndpoint due inherited Extensible attributes, got error: %s", diags))
		return
	}
...
func (r *PxgridEndpointResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update PxgridEndpoint due inherited Extensible attributes, got error: %s", diags))
		return
	}
...
func (r *PxgridEndpointResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	var diags diag.Diagnostics
	var data PxgridEndpointModel
	var goClientData misc.PxgridEndpoint
	...
	updateRes, _, err := r.client.MiscAPI.
		PxgridEndpointAPI.
		Update(ctx, resourceRef).
		PxgridEndpoint(goClientData).
		ReturnFieldsPlus(readableAttributesForPxgridEndpoint).
		ReturnAsObject(1).
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Import Failed", fmt.Sprintf("Unable to update PxgridEndpoint for import, got error: %s", err))
		return
	}

	res := updateRes.UpdatePxgridEndpointResponseAsObject.GetResult()

	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrsAll, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update PxgridEndpoint due inherited Extensible attributes for import, got error: %s", diags))
		return
	}

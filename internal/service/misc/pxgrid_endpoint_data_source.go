...
var _ datasource.DataSource = &PxgridEndpointDataSource{}

func NewPxgridEndpointDataSource() datasource.DataSource {
	return &PxgridEndpointDataSource{}
}

// PxgridEndpointDataSource defines the data source implementation.
type PxgridEndpointDataSource struct {
	client *niosclient.APIClient
}

func (d *PxgridEndpointDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "misc_pxgrid_endpoint"
}

type PxgridEndpointModelWithFilter struct {
...
func (m *PxgridEndpointModelWithFilter) FlattenResults(ctx context.Context, from []misc.PxgridEndpoint, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, PxgridEndpointAttrTypes, diags, FlattenPxgridEndpoint)
}

func (d *PxgridEndpointDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing PxgridEndpoint objects.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(PxgridEndpointResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *PxgridEndpointDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *PxgridEndpointDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data PxgridEndpointModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]misc.PxgridEndpoint, string, error) {
			...
			res := apiRes.ListPxgridEndpointResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListPxgridEndpointResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &DxlEndpointDataSource{}

func NewDxlEndpointDataSource() datasource.DataSource {
	return &DxlEndpointDataSource{}
}

// DxlEndpointDataSource defines the data source implementation.
type DxlEndpointDataSource struct {
	client *niosclient.APIClient
}

func (d *DxlEndpointDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "misc_dxl_endpoint"
}

type DxlEndpointModelWithFilter struct {
...
func (m *DxlEndpointModelWithFilter) FlattenResults(ctx context.Context, from []misc.DxlEndpoint, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, DxlEndpointAttrTypes, diags, FlattenDxlEndpoint)
}

func (d *DxlEndpointDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DxlEndpoint objects.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(DxlEndpointResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *DxlEndpointDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DxlEndpointDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data DxlEndpointModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]misc.DxlEndpoint, string, error) {
			...
			res := apiRes.ListDxlEndpointResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListDxlEndpointResponseObject.AdditionalProperties

...
func (r *DxlEndpointResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while create DxlEndpoint due inherited Extensible attributes, got error: %s", err))
		return
	}
...
func (r *DxlEndpointResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrs, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while reading DxlEndpoint due inherited Extensible attributes, got error: %s", diags))
		return
	}
...
func (r *DxlEndpointResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update DxlEndpoint due inherited Extensible attributes, got error: %s", diags))
		return
	}
...
func (r *DxlEndpointResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	var diags diag.Diagnostics
	var data DxlEndpointModel
	var goClientData misc.DxlEndpoint
	...
	updateRes, _, err := r.client.MiscAPI.
		...
		DxlEndpoint(goClientData).
		ReturnFieldsPlus(readableAttributesForDxlEndpoint).
		ReturnAsObject(1).
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Import Failed", fmt.Sprintf("Unable to update DxlEndpoint for import, got error: %s", err))
		return
	}

	res := updateRes.UpdateDxlEndpointResponseAsObject.GetResult()

	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrsAll, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update DxlEndpoint due inherited Extensible attributes for import, got error: %s", diags))
		return
	}

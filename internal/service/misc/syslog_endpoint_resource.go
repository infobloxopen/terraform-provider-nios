...
func (r *SyslogEndpointResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	var diags diag.Diagnostics
	var data SyslogEndpointModel
	var goClientData misc.SyslogEndpoint
	...
	updateRes, _, err := r.client.MiscAPI.
		...
		SyslogEndpoint(goClientData).
		ReturnFieldsPlus(readableAttributesForSyslogEndpoint).
		ReturnAsObject(1).
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Import Failed", fmt.Sprintf("Unable to update SyslogEndpoint for import, got error: %s", err))
		return
	}

	res := updateRes.UpdateSyslogEndpointResponseAsObject.GetResult()

	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrsAll, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update SyslogEndpoint due inherited Extensible attributes for import, got error: %s", diags))
		return
	}

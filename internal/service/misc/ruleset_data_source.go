...
// RulesetDataSource defines the data source implementation.
...
func (m *RulesetModelWithFilter) FlattenResults(ctx context.Context, from []misc.Ruleset, diags *diag.Diagnostics) {
	...
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RulesetAttrTypes, diags, FlattenRuleset)
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RulesetResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RulesetDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RulesetModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]misc.Ruleset, string, error) {
			...
			res := apiRes.ListRulesetResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRulesetResponseObject.AdditionalProperties

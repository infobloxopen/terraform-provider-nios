...
var _ datasource.DataSource = &FilterfingerprintDataSource{}

func NewFilterfingerprintDataSource() datasource.DataSource {
	return &FilterfingerprintDataSource{}
}

// FilterfingerprintDataSource defines the data source implementation.
type FilterfingerprintDataSource struct {
	client *niosclient.APIClient
}

func (d *FilterfingerprintDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_filterfingerprint"
}

type FilterfingerprintModelWithFilter struct {
...
func (m *FilterfingerprintModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Filterfingerprint, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, FilterfingerprintAttrTypes, diags, FlattenFilterfingerprint)
}

func (d *FilterfingerprintDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Filterfingerprint objects.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(FilterfingerprintResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *FilterfingerprintDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *FilterfingerprintDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data FilterfingerprintModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Filterfingerprint, string, error) {
			...
			res := apiRes.ListFilterfingerprintResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListFilterfingerprintResponseObject.AdditionalProperties

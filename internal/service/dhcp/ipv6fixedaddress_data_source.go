...
// Ipv6fixedaddressDataSource defines the data source implementation.
type Ipv6fixedaddressDataSource struct {
	client *niosclient.APIClient
}

func (d *Ipv6fixedaddressDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_ipv6fixedaddress"
}

type Ipv6fixedaddressModelWithFilter struct {
...
func (m *Ipv6fixedaddressModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Ipv6fixedaddress, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, Ipv6fixedaddressAttrTypes, diags, FlattenIpv6fixedaddress)
}

func (d *Ipv6fixedaddressDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DHCP IPV6 Fixed Addresses.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(Ipv6fixedaddressResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *Ipv6fixedaddressDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *Ipv6fixedaddressDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data Ipv6fixedaddressModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Ipv6fixedaddress, string, error) {
			...
			res := apiRes.ListIpv6fixedaddressResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListIpv6fixedaddressResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &FilternacDataSource{}

func NewFilternacDataSource() datasource.DataSource {
	return &FilternacDataSource{}
}

// FilternacDataSource defines the data source implementation.
type FilternacDataSource struct {
	client *niosclient.APIClient
}

func (d *FilternacDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_filternac"
}

type FilternacModelWithFilter struct {
...
func (m *FilternacModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Filternac, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, FilternacAttrTypes, diags, FlattenFilternac)
}

func (d *FilternacDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DHCP NAC Filters.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(FilternacResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *FilternacDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *FilternacDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data FilternacModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Filternac, string, error) {
			...
			res := apiRes.ListFilternacResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListFilternacResponseObject.AdditionalProperties

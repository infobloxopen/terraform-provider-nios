...
// Ipv6sharednetworkDataSource defines the data source implementation.
type Ipv6sharednetworkDataSource struct {
	client *niosclient.APIClient
}

func (d *Ipv6sharednetworkDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_ipv6sharednetwork"
}

type Ipv6sharednetworkModelWithFilter struct {
...
func (m *Ipv6sharednetworkModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Ipv6sharednetwork, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, Ipv6sharednetworkAttrTypes, diags, FlattenIpv6sharednetwork)
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(Ipv6sharednetworkResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *Ipv6sharednetworkDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *Ipv6sharednetworkDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data Ipv6sharednetworkModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Ipv6sharednetwork, string, error) {
			...
			res := apiRes.ListIpv6sharednetworkResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListIpv6sharednetworkResponseObject.AdditionalProperties

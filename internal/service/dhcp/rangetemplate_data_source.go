...
var _ datasource.DataSource = &RangetemplateDataSource{}

func NewRangetemplateDataSource() datasource.DataSource {
	return &RangetemplateDataSource{}
}

// RangetemplateDataSource defines the data source implementation.
type RangetemplateDataSource struct {
...
func (m *RangetemplateModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Rangetemplate, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RangetemplateAttrTypes, diags, FlattenRangetemplate)
}

func (d *RangetemplateDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DHCP Range Template Records.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RangetemplateResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RangetemplateDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RangetemplateDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RangetemplateModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Rangetemplate, string, error) {
			...
			res := apiRes.ListRangetemplateResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRangetemplateResponseObject.AdditionalProperties

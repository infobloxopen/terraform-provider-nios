...
var _ datasource.DataSource = &Ipv6rangeDataSource{}

func NewIpv6rangeDataSource() datasource.DataSource {
	return &Ipv6rangeDataSource{}
}

// Ipv6rangeDataSource defines the data source implementation.
type Ipv6rangeDataSource struct {
	client *niosclient.APIClient
}

func (d *Ipv6rangeDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_ipv6range"
}

type Ipv6rangeModelWithFilter struct {
...
func (m *Ipv6rangeModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Ipv6range, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, Ipv6rangeAttrTypes, diags, FlattenIpv6range)
}

func (d *Ipv6rangeDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Ipv6 Ranges.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(Ipv6rangeResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *Ipv6rangeDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *Ipv6rangeDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data Ipv6rangeModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Ipv6range, string, error) {
			...
			res := apiRes.ListIpv6rangeResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListIpv6rangeResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &FixedaddresstemplateDataSource{}

func NewFixedaddresstemplateDataSource() datasource.DataSource {
	return &FixedaddresstemplateDataSource{}
}

// FixedaddresstemplateDataSource defines the data source implementation.
type FixedaddresstemplateDataSource struct {
	client *niosclient.APIClient
}

func (d *FixedaddresstemplateDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_fixedaddresstemplate"
}

type FixedaddresstemplateModelWithFilter struct {
...
func (m *FixedaddresstemplateModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Fixedaddresstemplate, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, FixedaddresstemplateAttrTypes, diags, FlattenFixedaddresstemplate)
}

func (d *FixedaddresstemplateDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DHCP Fixed Address Templates.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(FixedaddresstemplateResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *FixedaddresstemplateDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *FixedaddresstemplateDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data FixedaddresstemplateModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Fixedaddresstemplate, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Fixedaddresstemplate by extattrs, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListFixedaddresstemplateResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListFixedaddresstemplateResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &MacfilteraddressDataSource{}

func NewMacfilteraddressDataSource() datasource.DataSource {
	return &MacfilteraddressDataSource{}
}

// MacfilteraddressDataSource defines the data source implementation.
type MacfilteraddressDataSource struct {
	client *niosclient.APIClient
}

func (d *MacfilteraddressDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_macfilteraddress"
}

type MacfilteraddressModelWithFilter struct {
...
func (m *MacfilteraddressModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Macfilteraddress, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, MacfilteraddressAttrTypes, diags, FlattenMacfilteraddress)
}

func (d *MacfilteraddressDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DHCP MAC Filter Addresses.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(MacfilteraddressResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *MacfilteraddressDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *MacfilteraddressDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data MacfilteraddressModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Macfilteraddress, string, error) {
			...
			res := apiRes.ListMacfilteraddressResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListMacfilteraddressResponseObject.AdditionalProperties

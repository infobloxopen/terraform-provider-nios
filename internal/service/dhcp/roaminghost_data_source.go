...
var _ datasource.DataSource = &RoaminghostDataSource{}

func NewRoaminghostDataSource() datasource.DataSource {
	return &RoaminghostDataSource{}
}

// RoaminghostDataSource defines the data source implementation.
type RoaminghostDataSource struct {
	client *niosclient.APIClient
}

func (d *RoaminghostDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_roaminghost"
}

type RoaminghostModelWithFilter struct {
...
func (m *RoaminghostModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Roaminghost, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, RoaminghostAttrTypes, diags, FlattenRoaminghost)
}

func (d *RoaminghostDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Roaminghost objects.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(RoaminghostResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *RoaminghostDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *RoaminghostDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data RoaminghostModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Roaminghost, string, error) {
			...
			res := apiRes.ListRoaminghostResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListRoaminghostResponseObject.AdditionalProperties

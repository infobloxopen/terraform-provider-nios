...
// Ipv6dhcpoptionspaceDataSource defines the data source implementation.
...
func (m *Ipv6dhcpoptionspaceModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Ipv6dhcpoptionspace, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, Ipv6dhcpoptionspaceAttrTypes, diags, FlattenIpv6dhcpoptionspace)
}

func (d *Ipv6dhcpoptionspaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing IPv6 DHCP Option Spaces.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(Ipv6dhcpoptionspaceResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *Ipv6dhcpoptionspaceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *Ipv6dhcpoptionspaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data Ipv6dhcpoptionspaceModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Ipv6dhcpoptionspace, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Ipv6dhcpoptionspace by filter, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListIpv6dhcpoptionspaceResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListIpv6dhcpoptionspaceResponseObject.AdditionalProperties

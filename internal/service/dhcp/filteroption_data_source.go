...
var _ datasource.DataSource = &FilteroptionDataSource{}

func NewFilteroptionDataSource() datasource.DataSource {
	return &FilteroptionDataSource{}
}

// FilteroptionDataSource defines the data source implementation.
type FilteroptionDataSource struct {
	client *niosclient.APIClient
}

func (d *FilteroptionDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_filteroption"
}

type FilteroptionModelWithFilter struct {
...
func (m *FilteroptionModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Filteroption, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, FilteroptionAttrTypes, diags, FlattenFilteroption)
}

func (d *FilteroptionDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Filteroptions",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(FilteroptionResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *FilteroptionDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *FilteroptionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data FilteroptionModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Filteroption, string, error) {
			...
			res := apiRes.ListFilteroptionResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListFilteroptionResponseObject.AdditionalProperties

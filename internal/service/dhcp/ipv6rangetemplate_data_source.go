...
// Ipv6rangetemplateDataSource defines the data source implementation.
...
func (m *Ipv6rangetemplateModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Ipv6rangetemplate, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, Ipv6rangetemplateAttrTypes, diags, FlattenIpv6rangetemplate)
}

func (d *Ipv6rangetemplateDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing IPV6 Range Templates.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(Ipv6rangetemplateResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *Ipv6rangetemplateDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *Ipv6rangetemplateDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data Ipv6rangetemplateModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Ipv6rangetemplate, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Ipv6rangetemplate by filter, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListIpv6rangetemplateResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListIpv6rangetemplateResponseObject.AdditionalProperties

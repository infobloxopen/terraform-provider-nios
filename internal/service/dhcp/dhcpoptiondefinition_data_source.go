...
// DhcpoptiondefinitionDataSource defines the data source implementation.
...
func (m *DhcpoptiondefinitionModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Dhcpoptiondefinition, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, DhcpoptiondefinitionAttrTypes, diags, FlattenDhcpoptiondefinition)
}

func (d *DhcpoptiondefinitionDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DHCP Option Definitions.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(DhcpoptiondefinitionResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *DhcpoptiondefinitionDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DhcpoptiondefinitionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data DhcpoptiondefinitionModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Dhcpoptiondefinition, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Dhcpoptiondefinition by filter, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListDhcpoptiondefinitionResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListDhcpoptiondefinitionResponseObject.AdditionalProperties

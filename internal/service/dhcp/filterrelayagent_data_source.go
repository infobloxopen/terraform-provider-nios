...
var _ datasource.DataSource = &FilterrelayagentDataSource{}

func NewFilterrelayagentDataSource() datasource.DataSource {
	return &FilterrelayagentDataSource{}
}

// FilterrelayagentDataSource defines the data source implementation.
type FilterrelayagentDataSource struct {
	client *niosclient.APIClient
}

func (d *FilterrelayagentDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_filterrelayagent"
}

type FilterrelayagentModelWithFilter struct {
...
func (m *FilterrelayagentModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Filterrelayagent, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, FilterrelayagentAttrTypes, diags, FlattenFilterrelayagent)
}

func (d *FilterrelayagentDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Filter Relay Agents.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(FilterrelayagentResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *FilterrelayagentDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *FilterrelayagentDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data FilterrelayagentModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Filterrelayagent, string, error) {
			...
			res := apiRes.ListFilterrelayagentResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListFilterrelayagentResponseObject.AdditionalProperties

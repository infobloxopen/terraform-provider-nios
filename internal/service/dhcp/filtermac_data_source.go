...
var _ datasource.DataSource = &FiltermacDataSource{}

func NewFiltermacDataSource() datasource.DataSource {
	return &FiltermacDataSource{}
}

// FiltermacDataSource defines the data source implementation.
type FiltermacDataSource struct {
	client *niosclient.APIClient
}

func (d *FiltermacDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dhcp_filtermac"
}

type FiltermacModelWithFilter struct {
...
func (m *FiltermacModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Filtermac, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, FiltermacAttrTypes, diags, FlattenFiltermac)
}

func (d *FiltermacDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DHCP Filter MACs based on specified filters and external attribute filters.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(FiltermacResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *FiltermacDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *FiltermacDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data FiltermacModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Filtermac, string, error) {
			...
			res := apiRes.ListFiltermacResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListFiltermacResponseObject.AdditionalProperties

...
func NewFingerprintDataSource() datasource.DataSource {
	return &FingerprintDataSource{}
}

// FingerprintDataSource defines the data source implementation.
type FingerprintDataSource struct {
...
type FingerprintModelWithFilter struct {
...
func (m *FingerprintModelWithFilter) FlattenResults(ctx context.Context, from []dhcp.Fingerprint, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, FingerprintAttrTypes, diags, FlattenFingerprint)
}

func (d *FingerprintDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DHCP Fingerprints.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(FingerprintResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *FingerprintDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data FingerprintModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dhcp.Fingerprint, string, error) {
			...
			res := apiRes.ListFingerprintResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListFingerprintResponseObject.AdditionalProperties

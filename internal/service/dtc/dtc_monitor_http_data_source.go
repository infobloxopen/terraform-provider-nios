...
var _ datasource.DataSource = &DtcMonitorHttpDataSource{}

func NewDtcMonitorHttpDataSource() datasource.DataSource {
	return &DtcMonitorHttpDataSource{}
}

// DtcMonitorHttpDataSource defines the data source implementation.
type DtcMonitorHttpDataSource struct {
	client *niosclient.APIClient
}

func (d *DtcMonitorHttpDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dtc_monitor_http"
}

type DtcMonitorHttpModelWithFilter struct {
...
func (m *DtcMonitorHttpModelWithFilter) FlattenResults(ctx context.Context, from []dtc.DtcMonitorHttp, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, DtcMonitorHttpAttrTypes, diags, FlattenDtcMonitorHttp)
}

func (d *DtcMonitorHttpDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DTC Monitors HTTP.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(DtcMonitorHttpResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *DtcMonitorHttpDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DtcMonitorHttpDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data DtcMonitorHttpModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dtc.DtcMonitorHttp, string, error) {
			...
			res := apiRes.ListDtcMonitorHttpResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListDtcMonitorHttpResponseObject.AdditionalProperties

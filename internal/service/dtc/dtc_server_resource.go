...
func (r *DtcServerResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if res.ExtAttrs == nil {
		resp.Diagnostics.AddError(
			...
			"Unable to read DtcServer because no extensible attributes were returned from the API.",
		)
		return
	}
	...
	if !ok {
		resp.Diagnostics.AddError(
			...
			"Unable to read DtcServer because terraform internal id does not exist.",
		)
		return
	}
...
func (r *DtcServerResource) addInternalIDToExtAttrs(ctx context.Context, data *DtcServerModel) error {

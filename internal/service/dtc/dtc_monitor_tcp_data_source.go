...
// DtcMonitorTcpDataSource defines the data source implementation.
type DtcMonitorTcpDataSource struct {
	client *niosclient.APIClient
}

func (d *DtcMonitorTcpDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dtc_monitor_tcp"
}

type DtcMonitorTcpModelWithFilter struct {
...
func (m *DtcMonitorTcpModelWithFilter) FlattenResults(ctx context.Context, from []dtc.DtcMonitorTcp, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, DtcMonitorTcpAttrTypes, diags, FlattenDtcMonitorTcp)
}

func (d *DtcMonitorTcpDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DTC TCP Monitors.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(DtcMonitorTcpResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *DtcMonitorTcpDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DtcMonitorTcpDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data DtcMonitorTcpModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dtc.DtcMonitorTcp, string, error) {
			...
			res := apiRes.ListDtcMonitorTcpResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListDtcMonitorTcpResponseObject.AdditionalProperties

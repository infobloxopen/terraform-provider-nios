...
var _ resource.ResourceWithValidateConfig = &DtcPoolResource{}
...
func (r *DtcPoolResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	updateRes, _, err := r.client.DTCAPI.
		...
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Import Failed", fmt.Sprintf("Unable to update DtcPool for import, got error: %s", err))
		return
	}

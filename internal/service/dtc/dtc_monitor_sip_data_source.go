...
var _ datasource.DataSource = &DtcMonitorSipDataSource{}

func NewDtcMonitorSipDataSource() datasource.DataSource {
	return &DtcMonitorSipDataSource{}
}

// DtcMonitorSipDataSource defines the data source implementation.
type DtcMonitorSipDataSource struct {
	client *niosclient.APIClient
}

func (d *DtcMonitorSipDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dtc_monitor_sip"
}

type DtcMonitorSipModelWithFilter struct {
...
func (m *DtcMonitorSipModelWithFilter) FlattenResults(ctx context.Context, from []dtc.DtcMonitorSip, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, DtcMonitorSipAttrTypes, diags, FlattenDtcMonitorSip)
}

func (d *DtcMonitorSipDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DTC SIP Monitors",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(DtcMonitorSipResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *DtcMonitorSipDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DtcMonitorSipDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data DtcMonitorSipModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dtc.DtcMonitorSip, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read DtcMonitorSip by extattrs, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListDtcMonitorSipResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListDtcMonitorSipResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &DtcRecordAaaaDataSource{}

func NewDtcRecordAaaaDataSource() datasource.DataSource {
	return &DtcRecordAaaaDataSource{}
}

// DtcRecordAaaaDataSource defines the data source implementation.
type DtcRecordAaaaDataSource struct {
	client *niosclient.APIClient
}

func (d *DtcRecordAaaaDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dtc_record_aaaa"
}

type DtcRecordAaaaModelWithFilter struct {
...
func (m *DtcRecordAaaaModelWithFilter) FlattenResults(ctx context.Context, from []dtc.DtcRecordAaaa, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, DtcRecordAaaaAttrTypes, diags, FlattenDtcRecordAaaa)
}

func (d *DtcRecordAaaaDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(DtcRecordAaaaResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *DtcRecordAaaaDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DtcRecordAaaaDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data DtcRecordAaaaModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dtc.DtcRecordAaaa, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read DtcRecordAaaa by filter, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListDtcRecordAaaaResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListDtcRecordAaaaResponseObject.AdditionalProperties

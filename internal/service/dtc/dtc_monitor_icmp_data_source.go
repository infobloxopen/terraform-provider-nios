...
var _ datasource.DataSource = &DtcMonitorIcmpDataSource{}

func NewDtcMonitorIcmpDataSource() datasource.DataSource {
	return &DtcMonitorIcmpDataSource{}
}

// DtcMonitorIcmpDataSource defines the data source implementation.
type DtcMonitorIcmpDataSource struct {
	client *niosclient.APIClient
}

func (d *DtcMonitorIcmpDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dtc_monitor_icmp"
}

type DtcMonitorIcmpModelWithFilter struct {
...
func (m *DtcMonitorIcmpModelWithFilter) FlattenResults(ctx context.Context, from []dtc.DtcMonitorIcmp, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, DtcMonitorIcmpAttrTypes, diags, FlattenDtcMonitorIcmp)
}

func (d *DtcMonitorIcmpDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DTC ICMP Monitors.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(DtcMonitorIcmpResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *DtcMonitorIcmpDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DtcMonitorIcmpDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data DtcMonitorIcmpModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dtc.DtcMonitorIcmp, string, error) {
			...
			res := apiRes.ListDtcMonitorIcmpResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListDtcMonitorIcmpResponseObject.AdditionalProperties

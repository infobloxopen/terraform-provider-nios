...
var _ datasource.DataSource = &DtcMonitorSnmpDataSource{}

func NewDtcMonitorSnmpDataSource() datasource.DataSource {
	return &DtcMonitorSnmpDataSource{}
}

// DtcMonitorSnmpDataSource defines the data source implementation.
type DtcMonitorSnmpDataSource struct {
	client *niosclient.APIClient
}

func (d *DtcMonitorSnmpDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "dtc_monitor_snmp"
}

type DtcMonitorSnmpModelWithFilter struct {
...
func (m *DtcMonitorSnmpModelWithFilter) FlattenResults(ctx context.Context, from []dtc.DtcMonitorSnmp, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, DtcMonitorSnmpAttrTypes, diags, FlattenDtcMonitorSnmp)
}

func (d *DtcMonitorSnmpDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing DTC SNMP Monitors.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(DtcMonitorSnmpResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *DtcMonitorSnmpDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DtcMonitorSnmpDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data DtcMonitorSnmpModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]dtc.DtcMonitorSnmp, string, error) {
			...
			res := apiRes.ListDtcMonitorSnmpResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListDtcMonitorSnmpResponseObject.AdditionalProperties

...
func (r *MsserverResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while create Msserver due inherited Extensible attributes, got error: %s", err))
		return
	}
...
func (r *MsserverResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrs, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while reading Msserver due inherited Extensible attributes, got error: %s", diags))
		return
	}
...
func (r *MsserverResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update Msserver due inherited Extensible attributes, got error: %s", diags))
		return
	}
...
func (r *MsserverResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	var diags diag.Diagnostics
	var data MsserverModel
	var goClientData microsoftserver.Msserver
	...
	updateRes, _, err := r.client.MicrosoftServerAPI.
		...
		Msserver(goClientData).
		ReturnFieldsPlus(readableAttributesForMsserver).
		ReturnAsObject(1).
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Import Failed", fmt.Sprintf("Unable to update Msserver for import, got error: %s", err))
		return
	}

	res := updateRes.UpdateMsserverResponseAsObject.GetResult()

	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrsAll, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update Msserver due inherited Extensible attributes for import, got error: %s", diags))
		return
	}

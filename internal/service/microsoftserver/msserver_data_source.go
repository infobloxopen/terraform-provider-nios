...
var _ datasource.DataSource = &MsserverDataSource{}

func NewMsserverDataSource() datasource.DataSource {
	return &MsserverDataSource{}
}

// MsserverDataSource defines the data source implementation.
type MsserverDataSource struct {
	client *niosclient.APIClient
}

func (d *MsserverDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "microsoftserver_msserver"
}

type MsserverModelWithFilter struct {
...
func (m *MsserverModelWithFilter) FlattenResults(ctx context.Context, from []microsoftserver.Msserver, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, MsserverAttrTypes, diags, FlattenMsserver)
}

func (d *MsserverDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(MsserverResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *MsserverDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *MsserverDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data MsserverModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]microsoftserver.Msserver, string, error) {
			...
			res := apiRes.ListMsserverResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListMsserverResponseObject.AdditionalProperties

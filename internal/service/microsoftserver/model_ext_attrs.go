...
func ExpandExtAttrs(ctx context.Context, extattrs types.Map, diags *diag.Diagnostics) *map[string]microsoftserver.ExtAttrs {
	...
	result := make(map[string]microsoftserver.ExtAttrs)

	for key, valStr := range extAttrsMap {
		parsedValue := utils.ParseInterfaceValue(valStr)
		result[key] = microsoftserver.ExtAttrs{Value: parsedValue}
	}
	return &result
}

func FlattenExtAttrs(ctx context.Context, planExtAttrs types.Map, extattrs *map[string]microsoftserver.ExtAttrs, diags *diag.Diagnostics) types.Map {
...
func RemoveInheritedExtAttrs(ctx context.Context, planExtAttrs types.Map, respExtAttrs map[string]microsoftserver.ExtAttrs) (*map[string]microsoftserver.ExtAttrs, types.Map, diag.Diagnostics) {
	var diags diag.Diagnostics
	var planMap map[string]microsoftserver.ExtAttrs
	extAttrsRespMap := make(map[string]microsoftserver.ExtAttrs, len(planExtAttrs.Elements()))
	extAttrsAllRespMap := make(map[string]microsoftserver.ExtAttrs)
	var extAttrAll types.Map

	if planExtAttrs.IsNull() || planExtAttrs.IsUnknown() {
		planMap = make(map[string]microsoftserver.ExtAttrs)
	} else {

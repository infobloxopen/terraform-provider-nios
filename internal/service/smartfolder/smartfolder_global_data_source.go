...
var _ datasource.DataSource = &SmartfolderGlobalDataSource{}

func NewSmartfolderGlobalDataSource() datasource.DataSource {
	return &SmartfolderGlobalDataSource{}
}

// SmartfolderGlobalDataSource defines the data source implementation.
type SmartfolderGlobalDataSource struct {
	client *niosclient.APIClient
}

func (d *SmartfolderGlobalDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "smartfolder_global"
}

type SmartfolderGlobalModelWithFilter struct {
...
func (m *SmartfolderGlobalModelWithFilter) FlattenResults(ctx context.Context, from []smartfolder.SmartfolderGlobal, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, SmartfolderGlobalAttrTypes, diags, FlattenSmartfolderGlobal)
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(SmartfolderGlobalResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *SmartfolderGlobalDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *SmartfolderGlobalDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data SmartfolderGlobalModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]smartfolder.SmartfolderGlobal, string, error) {
			...
			res := apiRes.ListSmartfolderGlobalResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListSmartfolderGlobalResponseObject.AdditionalProperties

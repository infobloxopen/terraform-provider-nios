...
type GridSyslogServersModel struct {
...
var GridSyslogServersAttrTypes = map[string]attr.Type{
...
var GridSyslogServersResourceSchemaAttributes = map[string]schema.Attribute{
...
func ExpandGridSyslogServers(ctx context.Context, o types.Object, diags *diag.Diagnostics) *grid.GridSyslogServers {
	if o.IsNull() || o.IsUnknown() {
		return nil
	}
	var m GridSyslogServersModel
...
func (m *GridSyslogServersModel) Expand(ctx context.Context, diags *diag.Diagnostics) *grid.GridSyslogServers {
	if m == nil {
		return nil
	}
	to := &grid.GridSyslogServers{
...
func FlattenGridSyslogServers(ctx context.Context, from *grid.GridSyslogServers, diags *diag.Diagnostics) types.Object {
	if from == nil {
		return types.ObjectNull(GridSyslogServersAttrTypes)
	}
	m := GridSyslogServersModel{}
	m.Flatten(ctx, from, diags)
	t, d := types.ObjectValueFrom(ctx, GridSyslogServersAttrTypes, m)
	diags.Append(d...)
	return t
}

func (m *GridSyslogServersModel) Flatten(ctx context.Context, from *grid.GridSyslogServers, diags *diag.Diagnostics) {
	if from == nil {
		return
	}
	if m == nil {

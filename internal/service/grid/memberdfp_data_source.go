...
var _ datasource.DataSource = &MemberdfpDataSource{}

func NewMemberdfpDataSource() datasource.DataSource {
	return &MemberdfpDataSource{}
}

// MemberdfpDataSource defines the data source implementation.
type MemberdfpDataSource struct {
	client *niosclient.APIClient
}

func (d *MemberdfpDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "grid_memberdfp"
}

type MemberdfpModelWithFilter struct {
...
func (m *MemberdfpModelWithFilter) FlattenResults(ctx context.Context, from []grid.Memberdfp, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, MemberdfpAttrTypes, diags, FlattenMemberdfp)
}

func (d *MemberdfpDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(MemberdfpResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *MemberdfpDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *MemberdfpDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data MemberdfpModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]grid.Memberdfp, string, error) {
			...
			res := apiRes.ListMemberdfpResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListMemberdfpResponseObject.AdditionalProperties

...
func (r *MemberdfpResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	apiRes, _, err := r.client.GridAPI.
		...
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create Memberdfp, got error: %s", err))
		return
	}

	res := apiRes.CreateMemberdfpResponseAsObject.GetResult()
	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrs, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while create Memberdfp due inherited Extensible attributes, got error: %s", err))
		return
	}
...
func (r *MemberdfpResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if stateExtAttrs == nil {
		resp.Diagnostics.AddError(
			"Missing Internal ID",
			"Unable to read Memberdfp because the internal ID (from extattrs_all) is missing or invalid.",
		)
		return
	}
	...
	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrs, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while reading Memberdfp due inherited Extensible attributes, got error: %s", diags))
		return
	}
...
func (r *MemberdfpResource) ReadByExtAttrs(ctx context.Context, data *MemberdfpModel, resp *resource.ReadResponse) bool {
	var diags diag.Diagnostics
	...
	apiRes, _, err := r.client.GridAPI.
		...
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Memberdfp by extattrs, got error: %s", err))
		return true
	}

	results := apiRes.ListMemberdfpResponseObject.GetResult()
...
func (r *MemberdfpResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update Memberdfp due inherited Extensible attributes, got error: %s", diags))
		return
	}
...
func (r *MemberdfpResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete Memberdfp, got error: %s", err))
		return
	}
}
...
func (r *MemberdfpResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	var diags diag.Diagnostics
	var data MemberdfpModel
	var goClientData grid.Memberdfp
	...
	updateRes, _, err := r.client.GridAPI.
		...
		Memberdfp(goClientData).
		ReturnFieldsPlus(readableAttributesForMemberdfp).
		ReturnAsObject(1).
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Import Failed", fmt.Sprintf("Unable to update Memberdfp for import, got error: %s", err))
		return
	}

	res := updateRes.UpdateMemberdfpResponseAsObject.GetResult()

	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrsAll, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update Memberdfp due inherited Extensible attributes for import, got error: %s", diags))
		return
	}

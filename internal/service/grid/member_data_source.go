...
type MemberModelWithFilter struct {
...
func (m *MemberModelWithFilter) FlattenResults(ctx context.Context, from []grid.Member, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, MemberAttrTypes, diags, FlattenMember)
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(MemberResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *MemberDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data MemberModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]grid.Member, string, error) {
			...
			res := apiRes.ListMemberResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListMemberResponseObject.AdditionalProperties

...
func ExpandExtAttr(ctx context.Context, tfMap types.Map, diags *diag.Diagnostics) *map[string]grid.ExtAttrs {
...
func (m *ExtAttrModel) Expand(ctx context.Context, diags *diag.Diagnostics) *grid.ExtAttrs {
	if m == nil {
		return nil
	}
	to := &grid.ExtAttrs{
...
func FlattenExtAttr(ctx context.Context, input map[string]grid.ExtAttrs, diags *diag.Diagnostics) types.Map {
...
func (m *ExtAttrModel) Flatten(ctx context.Context, from *grid.ExtAttrs, diags *diag.Diagnostics) {
	if from == nil {
		return
	}
	if m == nil {
...
func RemoveInheritedExtAttrs(ctx context.Context, planExtAttrs types.Map, respExtAttrs map[string]grid.ExtAttrs) (*map[string]grid.ExtAttrs, diag.Diagnostics) {
	var diags diag.Diagnostics
	newRespMap := make(map[string]grid.ExtAttrs, len(respExtAttrs))

...
var _ datasource.DataSource = &NamedaclDataSource{}

func NewNamedaclDataSource() datasource.DataSource {
	return &NamedaclDataSource{}
}

// NamedaclDataSource defines the data source implementation.
type NamedaclDataSource struct {
	client *niosclient.APIClient
}

func (d *NamedaclDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "acl_namedacl"
}

type NamedaclModelWithFilter struct {
...
func (m *NamedaclModelWithFilter) FlattenResults(ctx context.Context, from []acl.Namedacl, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, NamedaclAttrTypes, diags, FlattenNamedacl)
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(NamedaclResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *NamedaclDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *NamedaclDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data NamedaclModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]acl.Namedacl, string, error) {
			...
			res := apiRes.ListNamedaclResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListNamedaclResponseObject.AdditionalProperties

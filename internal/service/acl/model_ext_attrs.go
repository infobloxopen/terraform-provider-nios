...
func ExpandExtAttrs(ctx context.Context, extattrs types.Map, diags *diag.Diagnostics) *map[string]acl.ExtAttrs {
	...
	result := make(map[string]acl.ExtAttrs)

	for key, valStr := range extAttrsMap {
		parsedValue := utils.ParseInterfaceValue(valStr)
		result[key] = acl.ExtAttrs{Value: parsedValue}
	}
	return &result
}

func FlattenExtAttrs(ctx context.Context, planExtAttrs types.Map, extattrs *map[string]acl.ExtAttrs, diags *diag.Diagnostics) types.Map {
...
func RemoveInheritedExtAttrs(ctx context.Context, planExtAttrs types.Map, respExtAttrs map[string]acl.ExtAttrs) (*map[string]acl.ExtAttrs, types.Map, diag.Diagnostics) {
	var diags diag.Diagnostics
	var planMap map[string]acl.ExtAttrs
	extAttrsRespMap := make(map[string]acl.ExtAttrs, len(planExtAttrs.Elements()))
	extAttrsAllRespMap := make(map[string]acl.ExtAttrs)
	var extAttrAll types.Map

	if planExtAttrs.IsNull() || planExtAttrs.IsUnknown() {
		planMap = make(map[string]acl.ExtAttrs)
	} else {

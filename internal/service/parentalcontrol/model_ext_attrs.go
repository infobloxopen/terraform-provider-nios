...
func ExpandExtAttrs(ctx context.Context, extattrs types.Map, diags *diag.Diagnostics) *map[string]parentalcontrol.ExtAttrs {
	...
	result := make(map[string]parentalcontrol.ExtAttrs)

	for key, valStr := range extAttrsMap {
		parsedValue := utils.ParseInterfaceValue(valStr)
		result[key] = parentalcontrol.ExtAttrs{Value: parsedValue}
	}
	return &result
}

func FlattenExtAttrs(ctx context.Context, planExtAttrs types.Map, extattrs *map[string]parentalcontrol.ExtAttrs, diags *diag.Diagnostics) types.Map {
...
func RemoveInheritedExtAttrs(ctx context.Context, planExtAttrs types.Map, respExtAttrs map[string]parentalcontrol.ExtAttrs) (*map[string]parentalcontrol.ExtAttrs, types.Map, diag.Diagnostics) {
	var diags diag.Diagnostics
	var planMap map[string]parentalcontrol.ExtAttrs
	extAttrsRespMap := make(map[string]parentalcontrol.ExtAttrs, len(planExtAttrs.Elements()))
	extAttrsAllRespMap := make(map[string]parentalcontrol.ExtAttrs)
	var extAttrAll types.Map

	if planExtAttrs.IsNull() || planExtAttrs.IsUnknown() {
		planMap = make(map[string]parentalcontrol.ExtAttrs)
	} else {

...
func (r *ParentalcontrolSubscribersiteResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	var diags diag.Diagnostics
	var data ParentalcontrolSubscribersiteModel
	var goClientData parentalcontrol.ParentalcontrolSubscribersite
	...
	updateRes, _, err := r.client.ParentalControlAPI.
		ParentalcontrolSubscribersiteAPI.
		Update(ctx, resourceRef).
		ParentalcontrolSubscribersite(goClientData).
		ReturnFieldsPlus(readableAttributesForParentalcontrolSubscribersite).
		ReturnAsObject(1).
		Execute()
	if err != nil {
		resp.Diagnostics.AddError("Import Failed", fmt.Sprintf("Unable to update ParentalcontrolSubscribersite for import, got error: %s", err))
		return
	}

	res := updateRes.UpdateParentalcontrolSubscribersiteResponseAsObject.GetResult()

	res.ExtAttrs, data.ExtAttrsAll, diags = RemoveInheritedExtAttrs(ctx, data.ExtAttrsAll, *res.ExtAttrs)
	if diags.HasError() {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Error while update ParentalcontrolSubscribersite due inherited Extensible attributes for import, got error: %s", diags))
		return
	}

...
// AwsuserDataSource defines the data source implementation.
...
func (m *AwsuserModelWithFilter) FlattenResults(ctx context.Context, from []cloud.Awsuser, diags *diag.Diagnostics) {
	...
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, AwsuserAttrTypes, diags, FlattenAwsuser)
}

func (d *AwsuserDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing AWS Users.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(AwsuserResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *AwsuserDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *AwsuserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data AwsuserModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]cloud.Awsuser, string, error) {
			...
			if err != nil {
				resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Awsuser by extattrs, got error: %s", err))
				return nil, "", err
			}

			res := apiRes.ListAwsuserResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListAwsuserResponseObject.AdditionalProperties

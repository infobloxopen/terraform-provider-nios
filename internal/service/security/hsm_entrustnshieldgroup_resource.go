...
var _ resource.ResourceWithValidateConfig = &HsmEntrustnshieldgroupResource{}
...
func (r *HsmEntrustnshieldgroupResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Entrust nShield HSM group.",
...
func (r *HsmEntrustnshieldgroupResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	var data HsmEntrustnshieldgroupModel
	...
	if !data.Protection.IsNull() && !data.Protection.IsUnknown() && strings.ToUpper(data.Protection.ValueString()) == "SOFTCARD" {
		if data.CardName.IsNull() || data.CardName.IsUnknown() || data.CardName.ValueString() == "" {
			resp.Diagnostics.AddAttributeError(
				path.Root("card_name"),
				"Missing card_name for SOFTCARD protection",
		...
		if data.PassPhrase.IsNull() || data.PassPhrase.IsUnknown() || data.PassPhrase.ValueString() == "" {
			resp.Diagnostics.AddAttributeError(
				...
				"Missing pass_phrase for SOFTCARD protection",

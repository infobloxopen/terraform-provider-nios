...
var _ datasource.DataSource = &AdminroleDataSource{}

func NewAdminroleDataSource() datasource.DataSource {
	return &AdminroleDataSource{}
}

// AdminroleDataSource defines the data source implementation.
type AdminroleDataSource struct {
	client *niosclient.APIClient
}

func (d *AdminroleDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "security_admin_role"
}

type AdminroleModelWithFilter struct {
...
func (m *AdminroleModelWithFilter) FlattenResults(ctx context.Context, from []security.Adminrole, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, AdminroleAttrTypes, diags, FlattenAdminrole)
}

func (d *AdminroleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Admin Roles.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(AdminroleResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *AdminroleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data AdminroleModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]security.Adminrole, string, error) {
			...
			res := apiRes.ListAdminroleResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListAdminroleResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &AdmingroupDataSource{}

func NewAdmingroupDataSource() datasource.DataSource {
	return &AdmingroupDataSource{}
}

// AdmingroupDataSource defines the data source implementation.
type AdmingroupDataSource struct {
	client *niosclient.APIClient
}

func (d *AdmingroupDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "security_admin_group"
}

type AdmingroupModelWithFilter struct {
...
func (m *AdmingroupModelWithFilter) FlattenResults(ctx context.Context, from []security.Admingroup, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, AdmingroupAttrTypes, diags, FlattenAdmingroup)
}

func (d *AdmingroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Admin Groups.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(AdmingroupResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *AdmingroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *AdmingroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data AdmingroupModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]security.Admingroup, string, error) {
			...
			res := apiRes.ListAdmingroupResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListAdmingroupResponseObject.AdditionalProperties

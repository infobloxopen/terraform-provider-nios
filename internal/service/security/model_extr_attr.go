...
func ExpandExtAttr(ctx context.Context, tfMap types.Map, diags *diag.Diagnostics) *map[string]security.ExtAttrs {
...
func (m *ExtAttrModel) Expand(ctx context.Context, diags *diag.Diagnostics) *security.ExtAttrs {
	if m == nil {
		return nil
	}
	to := &security.ExtAttrs{
		Value: flex.ExpandString(m.Value),
	}
	return to
}

func FlattenExtAttr(ctx context.Context, input map[string]security.ExtAttrs, diags *diag.Diagnostics) types.Map {
...
func (m *ExtAttrModel) Flatten(ctx context.Context, from *security.ExtAttrs, diags *diag.Diagnostics) {
	if from == nil {
		return
	}
	if m == nil {
...
func RemoveInheritedExtAttrs(ctx context.Context, planExtAttrs types.Map, respExtAttrs map[string]security.ExtAttrs) (*map[string]security.ExtAttrs, diag.Diagnostics) {
	var diags diag.Diagnostics
	newRespMap := make(map[string]security.ExtAttrs, len(respExtAttrs))

...
var _ datasource.DataSource = &SnmpuserDataSource{}

func NewSnmpuserDataSource() datasource.DataSource {
	return &SnmpuserDataSource{}
}

// SnmpuserDataSource defines the data source implementation.
...
func (m *SnmpuserModelWithFilter) FlattenResults(ctx context.Context, from []security.Snmpuser, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, SnmpuserAttrTypes, diags, FlattenSnmpuser)
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(SnmpuserResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *SnmpuserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data SnmpuserModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]security.Snmpuser, string, error) {
			...
			res := apiRes.ListSnmpuserResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListSnmpuserResponseObject.AdditionalProperties

...
func (r *HsmThaleslunagroupResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages a Thales Luna HSM group.",
...
func (r *HsmThaleslunagroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// Process server cert file
	if !r.processServerCertFile(ctx, &data, &resp.Diagnostics) {
		return
	}
	...
	apiRes, _, err := r.client.SecurityAPI.
		...
		Create(ctx).
		HsmThaleslunagroup(*data.Expand(ctx, &resp.Diagnostics, true)).
...
func (r *HsmThaleslunagroupResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	apiRes, _, err := r.client.SecurityAPI.
		...
		HsmThaleslunagroup(*data.Expand(ctx, &resp.Diagnostics, false)).
		ReturnFieldsPlus(readableAttributesForHsmThaleslunagroup).
		ReturnAsObject(1).
		Execute()
	if err != nil {
...
// function that will process your server certificate file and return the token
func (r *HsmThaleslunagroupResource) processServerCertFile(ctx context.Context, data *HsmThaleslunagroupModel, diags *diag.Diagnostics) bool {
	if data.Thalesluna.IsNull() || data.Thalesluna.IsUnknown() {
		return true
	}
	...
	var lunaModels []HsmThaleslunagroupThaleslunaModel
	diagResult := data.Thalesluna.ElementsAs(ctx, &lunaModels, false)
	...
	// Loop over ALL luna devices and process the server certificate
	for i, lunaModel := range lunaModels {
		if !lunaModel.ServerCertFilePath.IsNull() && !lunaModel.ServerCertFilePath.IsUnknown() {
			filePath := lunaModel.ServerCertFilePath.ValueString()
			token, err := utils.UploadFileWithToken(ctx, baseUrl, filePath, username, password)
			if err != nil {
				diags.AddError(
					"Client Error",
					fmt.Sprintf("Unable to process server certificate file %s, got error: %s", filePath, err),
				)
				return false
			}
			lunaModels[i].ServerCert = types.StringValue(token)
		}
	}

	listValue, diagResult := types.ListValueFrom(ctx, types.ObjectType{AttrTypes: HsmThaleslunagroupThaleslunaAttrTypes}, lunaModels)
	diags.Append(diagResult...)
	if diags.HasError() {
		return false
	}

	data.Thalesluna = listValue
	return true
}

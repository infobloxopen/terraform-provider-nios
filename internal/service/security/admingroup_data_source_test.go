...
func TestAccAdmingroupDataSource_Filters(t *testing.T) {
	dataSourceName := "data.nios_security_admin_group.test"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAdmingroupDataSourceConfigFilters(name),
...
func TestAccAdmingroupDataSource_ExtAttrFilters(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAdmingroupDataSourceConfigExtAttrFilters(name, acctest.RandomName()),
				Check: resource.ComposeTestCheckFunc(
					append([]resource.TestCheckFunc{
						testAccCheckAdmingroupExists(context.Background(), resourceName, &v),
					}, testAccCheckAdmingroupResourceAttrPair(resourceName, dataSourceName)...)...,
...
func testAccAdmingroupDataSourceConfigFilters(name string) string {
	return fmt.Sprintf(`
resource "nios_security_admin_group" "test" {
  name = %q
}

data "nios_security_admin_group" "test" {
  filters = {
	 name = nios_security_admin_group.test.name
  }
}
`, name)
}

func testAccAdmingroupDataSourceConfigExtAttrFilters(name, extAttrsValue string) string {
	return fmt.Sprintf(`
resource "nios_security_admin_group" "test" {
  name = %q
  extattrs = {
...
data "nios_security_admin_group" "test" {
  extattrfilters = {
	Site = nios_security_admin_group.test.extattrs.Site
  }
}
`, name, extAttrsValue)
}

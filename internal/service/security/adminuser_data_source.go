...
// AdminuserDataSource defines the data source implementation.
...
type AdminuserModelWithFilter struct {
...
func (m *AdminuserModelWithFilter) FlattenResults(ctx context.Context, from []security.Adminuser, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, AdminuserAttrTypes, diags, FlattenAdminuser)
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(AdminuserResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *AdminuserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data AdminuserModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]security.Adminuser, string, error) {
			...
			res := apiRes.ListAdminuserResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListAdminuserResponseObject.AdditionalProperties

...
var _ datasource.DataSource = &FtpuserDataSource{}

func NewFtpuserDataSource() datasource.DataSource {
	return &FtpuserDataSource{}
}

// FtpuserDataSource defines the data source implementation.
type FtpuserDataSource struct {
	client *niosclient.APIClient
}

func (d *FtpuserDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + "security_ftpuser"
}

type FtpuserModelWithFilter struct {
...
func (m *FtpuserModelWithFilter) FlattenResults(ctx context.Context, from []security.Ftpuser, diags *diag.Diagnostics) {
	if len(from) == 0 {
		return
	}
	m.Result = flex.FlattenFrameworkListNestedBlock(ctx, from, FtpuserAttrTypes, diags, FlattenFtpuser)
}

func (d *FtpuserDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves a specific FTP users based on filters.",
		Attributes: map[string]schema.Attribute{
			...
			"result": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceAttributeMap(FtpuserResourceSchemaAttributes, &resp.Diagnostics),
				},
				Computed: true,
			},
...
func (d *FtpuserDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *FtpuserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data FtpuserModelWithFilter
	pageCount := 0
	...
	allResults, err := utils.ReadWithPages(
		func(pageID string, maxResults int32) ([]security.Ftpuser, string, error) {
			...
			res := apiRes.ListFtpuserResponseObject.GetResult()
			tflog.Info(ctx, fmt.Sprintf("Page %d : Retrieved %d results", pageCount, len(res)))

			// Check for next page ID in additional properties
			additionalProperties := apiRes.ListFtpuserResponseObject.AdditionalProperties

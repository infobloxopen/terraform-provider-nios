...
variable "ami_id" {
  description = "AMI ID for NIOS instance. If not provided, will search for latest NIOS AMI"
  type        = string
  default     = null
}
...
variable "private_ips_count_eth2" {
  description = "Number of private IPs to assign to ETH2 (HA interface). Set 1 for secondary IP (VIP) for HA, 0 for no secondary IP"
  type        = number
  default     = 0
}
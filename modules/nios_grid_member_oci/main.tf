# NIOS on OCI — Instance Grid Deployment
# Deployment order
#   1. Object Storage Bucket      (skipped when create_bucket = false)
#   2. Upload NIOS QCOW2          to the bucket
#   3. Custom Image               imported from the bucket object
#   4. Compute Instances     using the custom image
#   5. Secondary VNICs (LAN1)     attached to each instance
...
# 2. Upload NIOS QCOW2 to Object Storage
# For large files use the OCI CLI to upload out-of-band:
#   curl -L "<url>" | oci os object put \
#     --namespace <ns> --bucket-name <bucket> \
#     --name <object> --file - --force
...
# 3. Custom Image — Import from Object Storage
...
# 5. Secondary VNICs — LAN1 (one per instance)
...
# 6. Reporting Block Volume (optional — member instance only)
